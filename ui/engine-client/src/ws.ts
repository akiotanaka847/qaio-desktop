 ...
 * ```
 *
 * The engine topic map (see `engine/qaio-engine-protocol/src/lib.rs::event_topic`):
 * - `*` — firehose; matches every topic below (see
 *    `engine/qaio-engine-server/src/ws.rs::is_subscribed`)
...
import type { QaioClient } from "./client";
...
export class EngineWebSocket {
  ...
  constructor(private client: QaioClient) {}
  ...
  /**
   * Typed event handler — called once per `QaioEvent` (kind:"event"
