/**
 * Wire types mirroring `engine/qaio-engine-protocol/src/lib.rs` and
 * domain DTOs from `engine/qaio-engine-core`.
 *
...
/**
 * Where Qaio found the CLI binary backing a provider. Surfaced so
 * the UI can label whether the user is talking to a copy Qaio shipped
 * (`bundled`), one Qaio downloaded for them (`managed`), one already
 * on their PATH (`path`), or nothing at all (`missing`).
 *
 * Mirrors the Rust `qaio_engine_core::provider::InstallSource` enum
 ...
 *
 * Keep in sync with `qaio-engine-core::preferences` constants.
 */
