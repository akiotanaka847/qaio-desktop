/**
 * `QaioClient` — thin fetch wrapper keyed by `{baseUrl, token}`.
 *
 * Usage:
 * ```ts
 * const engine = new QaioClient({ baseUrl: "http://127.0.0.1:7777", token });
 * const workspaces = await engine.listWorkspaces();
 * ```
 *
 * One method per REST route. DTOs mirror `engine/qaio-engine-core`.
 */
...
export interface QaioClientOptions {
  baseUrl: string;
  token: string;
}

export class QaioClient {
  private readonly baseUrl: string;
  private readonly token: string;

  constructor(opts: QaioClientOptions) {
  ...
  private async toError(res: Response): Promise<QaioEngineError> {
    const err = (await res.json().catch(() => null)) as ErrorBody | null;
    return new QaioEngineError(res.status, err);
  }
...
export class QaioEngineError extends Error {
   ...
   * instead of parsing `message`. See `engine/qaio-engine-core/src/skills.rs`
...
export function isQaioEngineError(e: unknown): e is QaioEngineError {
  return e instanceof QaioEngineError;
}
