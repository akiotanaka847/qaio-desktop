...
import { cn, Button } from "@qaio-ai/core"
...
export function FilesBrowser({
  ...
  return (
    <div
      className="relative flex flex-col overflow-hidden bg-card border border-border rounded-xl h-full"
      {...(onFilesDropped || onMove ? dragHandlers : {})}
    >
      <div className="h-[24px] shrink-0 border-b border-border bg-secondary select-none flex items-center rounded-t-xl">
      ...
      >
        {loading ? (
          ...
          <>
            <div className="shrink-0 [&>:nth-child(even)]:bg-secondary [&>:nth-child(even)]:rounded-lg">
              {creatingFolder && (
                <NewFolderInput
      ...
      <div className="h-[22px] shrink-0 border-t border-border bg-secondary select-none flex items-center justify-between px-3 rounded-b-xl">
  ...
  return (
    <div ref={containerRef} className="flex-1 min-h-0">
      {Array.from({ length: count }, (_, i) => (
        <div
          key={i}
          className={cn("h-[24px]", (startIndex + i) % 2 === 1 && "bg-secondary rounded-lg")}
          onClick={onDeselect}
  ...
  return createPortal(
    <>
      ...
      >
        <button
          onClick={onNewFolder}
          className="w-full text-left px-3 py-1.5 text-[13px] hover:bg-accent hover:text-accent-foreground rounded-md mx-0.5"
