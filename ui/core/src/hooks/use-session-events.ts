/**
 * Base hook for subscribing to qaio-tauri backend events.
 *
 * Uses a ref-based handler pattern to avoid the race condition in
 * useQaioEvent (where handler recreation tears down and re-registers
 * the listener, causing missed events).
 *
 * Apps pass their own `listen` function from `@tauri-apps/api/event`
 * so @qaio-ai/core doesn't need a build-time dependency on Tauri.
 *
...
import type { QaioEvent } from "../types";
...
export interface SessionEventsHandlers {
  ...
  onEvent?: (event: QaioEvent) => void;
}

/**
 * Subscribe to "qaio-event" from the Rust backend.
 *
 * Core events handled:
...
export function useSessionEvents(handlers: SessionEventsHandlers): void {
  ...
  useEffect(() => {
    const unlisten = ref.current.listen<QaioEvent>("qaio-event", (event) => {
      const h = ref.current;
