 ...
 * 2. `import.meta.env.VITE_QAIO_ENGINE_BASE` + `..._TOKEN` — build-time
...
export function resolveEngineConfig(): EngineConfig {
  ...
  const envBase = (import.meta as ImportMetaWithEnv).env?.VITE_QAIO_ENGINE_BASE;
  const envToken = (import.meta as ImportMetaWithEnv).env?.VITE_QAIO_ENGINE_TOKEN;
...
interface ImportMetaWithEnv {
  env?: {
    VITE_QAIO_ENGINE_BASE?: string;
    VITE_QAIO_ENGINE_TOKEN?: string;
  };
}
