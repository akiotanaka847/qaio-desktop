/**
 * Singleton QaioClient wired to the config in localStorage.
 *
 * The client is the ONLY dependency on Qaio — the rest of the app is
...
export function connectEngine(cfg: EngineConfig): QaioClient {
  _client = new QaioClient(cfg);
  _activeConfig = cfg;
...
export function getClient(): QaioClient {
  if (!_client) {
