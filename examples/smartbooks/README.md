# SmartBooks — custom frontend on qaio-engine

A reference consumer of the Qaio Engine that looks nothing like the
Qaio App. Same backend, different brand. Drop a bank-statement PDF,
...
React/Vue/Svelte/native app can talk to `qaio-engine` over HTTP+WS
using `@qaio-ai/engine-client`. No Tauri, no Qaio App look, no
...
- Vite + React 19 — hand-rolled CSS, zero `@qaio-ai/*` UI packages
- `@qaio-ai/engine-client` — the only Qaio dependency
...
cargo build --release -p qaio-engine-server --bin qaio-engine

# Pick any token + port. Use a scratch home dir so you don't mix with
# real Qaio App data.
QAIO_HOME=/tmp/smartbooks-home \
QAIO_DOCS=/tmp/smartbooks-docs \
QAIO_BIND=127.0.0.1:7777 \
QAIO_ENGINE_TOKEN=dev-token \
QAIO_NO_PARENT_WATCHDOG=1 \
  ./target/release/qaio-engine
```

First line of stdout:

```
QAIO_ENGINE_LISTENING port=7777 token=dev-token
```
...
2. Vite build-time env: `VITE_QAIO_ENGINE_BASE` +
   `VITE_QAIO_ENGINE_TOKEN` (see `.env.example`) — the path a
...
```
+--------------------+         HTTP + WebSocket         +---------------------+
|  SmartBooks (this) |  <------------------------------> |   qaio-engine    |
|  React + Vite      |   @qaio-ai/engine-client SDK  |  (Rust, axum)       |
+--------------------+                                   +----------+----------+
                                                                    |
                                                            spawns subprocess
                                                                    v
...
| [src/lib/engine.ts](src/lib/engine.ts) | `QaioClient` + `EngineWebSocket` singletons |
...
| [src/lib/feed.ts](src/lib/feed.ts) | `FeedItem` / `QaioEvent` types + message reducer |
...
| [src/styles.css](src/styles.css) | SmartBooks brand — warm ink + serif, deliberately unlike Qaio |
...
custom frontend consuming qaio-engine will need them too.
...
```bash
# Kill processes
kill $(pgrep qaio-engine)  # or Ctrl+C in the engine's terminal
...
For richer integrations, study the `@qaio-ai/engine-client` surface
