...
async fn forward_events(
    mut events: tokio::sync::broadcast::Receiver<QaioEvent>,
    ...
    loop {
        ...
        let is_session_status = matches!(event, QaioEvent::SessionStatus { .. });
        let is_low_sev = matches!(
            &event,
            QaioEvent::FeedItem { item, .. } if is_low_severity_feed(item),
        );
...
mod tests {
    ...
    //! invisible to the Qaio desktop app for an entire release cycle.
