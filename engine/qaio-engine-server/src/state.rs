...
use qaio_engine_core::routines::scheduler::RoutineSchedulerState;
use qaio_engine_core::{attachments::AttachmentUploadStore, paths::EnginePaths, EngineState};
use qaio_file_watcher::WatcherState;
use qaio_tunnel::{TunnelIdentity, TunnelRuntimeState};
...
impl ServerState {
    /// Initialise state with a file-backed DB at `<home>/db/qaio.db`.
    ///
    /// `tunnel_identity` is the relay handshake result — `Some` once
    /// `qaio_tunnel::ensure` returns, `None` if the first-boot
    ...
    pub async fn new(
        ...
        let db_path = config.home_dir.join("db").join("qaio.db");
