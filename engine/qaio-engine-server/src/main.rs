//! `qaio-engine` binary entry point.
//!
//! Reads config from env, binds a TCP listener, writes `engine.json` to the
//! Qaio home dir so the desktop supervisor can discover `{port, pid,
//! token_hash, version}`, and serves the full router.

use qaio_engine_protocol::{ENGINE_VERSION, PROTOCOL_VERSION};
use qaio_engine_server::{build_router, ServerConfig, ServerState};
use qaio_tunnel::{EngineEndpoint, TunnelClient, TunnelConfig};
use serde::Serialize;
...
async fn main() {
    ...
    // the main thread until finished and delayed the `QAIO_ENGINE_LISTENING`
    // banner — so the Tauri supervisor saw a longer startup and the
    // "Starting Qaio engine…" splash lingered. Kick it off on a
    ...
    let path_init = tokio::task::spawn_blocking(|| {
        qaio_terminal_manager::claude_path::init();
    });
    ...
    println!(
        "QAIO_ENGINE_LISTENING port={} token={}",
        actual.port(),
        cfg.token
    );
    tracing::info!(
        "qaio-engine {} (protocol v{}) listening on {}",
        ENGINE_VERSION,
        PROTOCOL_VERSION,
        actual
    );
    ...
    let tunnel_identity = match qaio_tunnel::ensure(&cfg.home_dir, &cfg.tunnel_url).await {
        Ok(identity) => {
            tracing::info!(
                target: "qaio_tunnel",
    ...
    // `QaioEvent`s for the frontend to react to and never block the
...
fn spawn_cli_lifecycles(state: Arc<ServerState>) {
    {
        ...
        tokio::spawn(async move {
            qaio_composio::lifecycle::ensure_and_upgrade(sink, db).await;
        });
    }
    {
        let sink = state.engine.events.clone();
        let db = state.engine.db.clone();
        tokio::spawn(async move {
            qaio_claude_installer::ensure_and_upgrade(sink, db).await;
        });
    }
}
...
fn init_tracing() {
    ...
    let filter =
        EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info,qaio=debug"));
