...
async fn mint_pairing(State(st): State<Arc<ServerState>>) -> Result<Json<PairingCode>, ApiError> {
    let Some(runtime) = &st.tunnel_runtime else {
        return Err(ApiError(CoreError::Unavailable(
            "Tunnel allocation hasn't completed yet. The engine's first boot needs network to reach the Qaio relay, then pairing becomes available on the next restart.".into(),
        )));
    };
    let snapshot = runtime.snapshot();
    if !snapshot.connected {
        return Err(ApiError(CoreError::Unavailable(
            "Phone access is still connecting. Keep Qaio open and try again in a moment.".into(),
        )));
    }
    let code = st
        .mobile_access
...
async fn reset_access(State(st): State<Arc<ServerState>>) -> Result<Json<PairingCode>, ApiError> {
    let Some(runtime) = &st.tunnel_runtime else {
        return Err(ApiError(CoreError::Unavailable(
            "Tunnel allocation hasn't completed yet. Restart Qaio with an internet connection before resetting phone access.".into(),
        )));
    };
    let snapshot = runtime.snapshot();
    let code = st
...
fn pairing_error(err: qaio_tunnel::PairError) -> ApiError {
