...
//! CLI that Qaio downloads on first launch (see
//! `qaio_claude_installer`).
//!
...
struct ClaudeStatus {
    ...
    /// Qaio upgrade that bumps the pinned version.
...
async fn cli_installed(State(_st): State<Arc<ServerState>>) -> Json<CliInstalled> {
    Json(CliInstalled {
        installed: qaio_claude_installer::is_installed(),
    })
}

async fn status(State(st): State<Arc<ServerState>>) -> Json<ClaudeStatus> {
    let installed = qaio_claude_installer::is_installed();
    let install_path = qaio_claude_installer::cli_path()
        .to_string_lossy()
        .to_string();

    let pinned_version = qaio_cli_bundle::load_bundled_manifest()
...
async fn install(State(st): State<Arc<ServerState>>) -> Result<(), ApiError> {
    ...
    tokio::spawn(async move {
        sink.emit(qaio_ui_events::QaioEvent::ClaudeCliInstalling { progress_pct: 0 });
        let sink_for_progress = sink.clone();
        let result =
            qaio_claude_installer::install(&entry, move |pct| {
                sink_for_progress
                    .emit(qaio_ui_events::QaioEvent::ClaudeCliInstalling { progress_pct: pct });
            })
            .await;
        match result {
            Ok(_) => {
                ...
                sink.emit(qaio_ui_events::QaioEvent::ClaudeCliReady);
            }
            Err(e) => {
                sink.emit(qaio_ui_events::QaioEvent::ClaudeCliFailed { message: e });
            }
        }
    });
