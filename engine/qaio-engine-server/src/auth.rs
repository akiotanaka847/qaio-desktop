...
//! 1. The **bootstrap token** in `ServerConfig::token` (env `QAIO_ENGINE_TOKEN`
...
use qaio_engine_protocol::{ErrorBody, ErrorCode, ErrorDetail};
...
fn extract_bearer(req: &Request) -> Option<String> {
        ...
        .and_then(|v| v.to_str().ok())
    {
        for p in v.split(',').map(|p| p.trim()) {
            if let Some(t) = p.strip_prefix("qaio-bearer.") {
