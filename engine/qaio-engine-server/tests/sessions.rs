...
async fn ws_only_delivers_subscribed_topics() {
    ...
    state.events.emit(QaioEvent::FeedItem {
        agent_path: "/a".into(),
...
async fn ws_unsub_stops_delivery() {
    ...
    state.events.emit(QaioEvent::SessionStatus {
        agent_path: "/a".into(),
        session_key: "k".into(),
        status: "running".into(),
        error: None,
...
async fn rest_cancel_nonexistent_session_returns_false() {
    ...
    let encoded_path = urlencoding::encode("/tmp/qaio-test-agent");
...
async fn rest_cancel_existing_session_emits_events() {
    ...
    let encoded_path = urlencoding::encode("/tmp/qaio-fake");
