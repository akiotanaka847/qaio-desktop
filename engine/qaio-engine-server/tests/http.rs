...
async fn health_authorized_with_bearer() {
    ...
    assert!(res.headers().get("x-qaio-engine-version").is_some());
...
mod ws_helpers {
    ...
    use qaio_engine_protocol::{ClientRequest, EngineEnvelope, EnvelopeKind};
    ...
    pub async fn spawn_server() -> (SocketAddr, String, qaio_ui_events::BroadcastEventSink) {
...
async fn ws_firehose_subscriber_receives_scoped_events() {
    use qaio_ui_events::{EventSink, QaioEvent};
...
async fn ws_no_subscription_receives_nothing() {
    ...
    events.emit(QaioEvent::ComposioCliReady);
    events.emit(QaioEvent::ActivityChanged {
