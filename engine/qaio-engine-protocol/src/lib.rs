...
//! third-party) speaks this protocol to talk to `qaio-engine`.
...
pub const HEADER_ENGINE_VERSION: &str = "X-Qaio-Engine-Version";
...
pub enum EnvelopeKind {
    /// Server-push event (payload = `QaioEvent` or `LagMarker`).
    Event,
...
/// Helper: build an event envelope from a QaioEvent.
pub fn event_envelope(event: &QaioEvent) -> EngineEnvelope {
    EngineEnvelope {
        v: PROTOCOL_VERSION,
...
/// Map a `QaioEvent` to its WS topic.
///
...
pub fn event_topic(event: &QaioEvent) -> String {
    match event {
        QaioEvent::FeedItem { session_key, .. }
        | QaioEvent::SessionStatus { session_key, .. } => format!("session:{session_key}"),
        QaioEvent::AuthRequired { .. } => "auth".into(),
        QaioEvent::Toast { .. } | QaioEvent::CompletionToast { .. } => "toast".into(),
        QaioEvent::EventReceived { .. } | QaioEvent::EventProcessed { .. } => {
            "events".into()
        }
        QaioEvent::HeartbeatFired { .. } | QaioEvent::CronFired { .. } => {
            "scheduler".into()
        }
        QaioEvent::RoutinesChanged { agent_path }
        | QaioEvent::RoutineRunsChanged { agent_path } => format!("routines:{agent_path}"),
        QaioEvent::ActivityChanged { agent_path }
        | QaioEvent::SkillsChanged { agent_path }
        ...
        | QaioEvent::ProviderLoginDeviceCode { .. } => "providers".into(),
...
pub fn is_low_severity_feed(item: &qaio_terminal_manager::FeedItem) -> bool {
    matches!(
        item,
        qaio_terminal_manager::FeedItem::AssistantTextStreaming(_)
            | qaio_terminal_manager::FeedItem::ThinkingStreaming(_)
    )
}
...
mod tests {
    ...
    fn event_topic_session_scoped() {
        let ev = QaioEvent::FeedItem {
            agent_path: "/a".into(),
            session_key: "k1".into(),
            item: qaio_terminal_manager::FeedItem::AssistantText("hi".into()),
        };
        assert_eq!(event_topic(&ev), "session:k1");

        let ev = QaioEvent::SessionStatus {
            agent_path: "/a".into(),
            session_key: "k1".into(),
            status: "running".into(),
            error: None,
    ...
    fn event_topic_singletons() {
        let ev = QaioEvent::Toast { message: "x".into(), variant: "info".into() };
        assert_eq!(event_topic(&ev), "toast");
        assert_eq!(event_topic(&QaioEvent::ComposioCliReady), "composio");
    }

    #[test]
    fn low_severity_feed_detection() {
        use qaio_terminal_manager::FeedItem;
