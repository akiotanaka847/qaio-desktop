...
//!
//! Qaio's composio integration is a thin wrapper around the upstream
...
//!    `qaio_cli_bundle`. No install step — `install()` is a no-op
...
//!    with developers running `cargo run -p qaio-engine-server`
...
//! cache). Qaio only invokes the binary and reads structured
...
/// production builds resolve via `qaio_cli_bundle` instead.
...
pub fn cli_path() -> PathBuf {
    qaio_cli_bundle::bundled_composio_binary().unwrap_or_else(standalone_cli_path)
}

/// Resolve the active install directory (the parent of `cli_path`,
/// containing the binary plus its sibling files).
pub fn install_dir() -> PathBuf {
    qaio_cli_bundle::bundled_composio_dir().unwrap_or_else(standalone_install_dir)
}

/// True if the bundled CLI is shipped with this build of Qaio.
/// Lifecycle code uses this to decide whether to skip the install /
/// upgrade dance entirely.
pub fn is_bundled() -> bool {
    qaio_cli_bundle::bundled_composio_binary().is_some()
}
...
/// the bundled CLI is shipped with Qaio this is a no-op that returns
...
pub async fn install() -> Result<PathBuf, String> {
    if let Some(p) = qaio_cli_bundle::bundled_composio_binary() {
        tracing::info!(
...
pub async fn install() -> Result<PathBuf, String> {
    ...
    let result = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || {
            let tmp_out = std::env::temp_dir().join("qaio-composio-install-stdout.log");
            let tmp_err = std::env::temp_dir().join("qaio-composio-install-stderr.log");
...
/// rest of `qaio-composio` doesn't read `$HOME` directly — that env
...
mod tests {
    ...
    /// that fake an `.app` layout in `qaio-cli-bundle::tests`.
    #[test]
