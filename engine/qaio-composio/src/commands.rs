...
//!
//! Qaio's composio support is powered by the `composio` CLI. Each
//! function here is a thin wrapper around `cli::*` functions
//! that shell out to the binary at `~/.composio/composio`.
//!
//! Tauri command decorators live in the adapter crate (`qaio-tauri`),
...
/// Current state of Qaio's composio integration.
