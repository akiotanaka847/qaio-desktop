...
struct PendingOAuth {
    ...
    /// Claude Code's spawned MCP clients (running inside Qaio agents)
...
async fn run_callback_listener() -> Result<(), String> {
    ...
    send_response(
        ...
        "You can close this tab and return to Qaio.",
    )
    .await;
...
async fn discover_oauth_metadata() -> Result<DiscoveryInfo, String> {
    ...
    let init_body = serde_json::json!({
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": { "name": "qaio", "version": "0" }
        }
    });
...
/// Fallback key used when Qaio needs to create a `mcpOAuth.composio*`
/// entry and no `composio*` key exists yet. Normally Qaio writes into
...
/// that the models running *inside* Qaio agents use — can find the
/// tokens under its expected key.
const QAIO_COMPOSIO_KEY: &str = "composio|qaio";
...
/// `composio|3cef502fa536d618`) and one that Qaio created. Writing to
/// all of them means that both Qaio's own Tauri commands AND the Claude
/// Code sessions that Qaio spawns as agents will find a valid token
/// under whichever key they look up.
///
/// If no `composio*` entry exists at all, we create one under
/// `QAIO_COMPOSIO_KEY` with the full `discoveryState` shape that Claude
...
fn update_keychain_token(
    ...
    // If nothing exists yet, seed Qaio's own entry — otherwise Claude
    // Code's spawned MCP clients would have nothing to read.
    let target_keys: Vec<String> = if existing_keys.is_empty() {
        mcp_oauth.insert(
            QAIO_COMPOSIO_KEY.to_string(),
            serde_json::json!({}),
        );
        vec![QAIO_COMPOSIO_KEY.to_string()]
    } else {
        existing_keys
    };
