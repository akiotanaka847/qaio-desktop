...
//! `composio_auth.rs`. Everything Qaio needs — auth, linking apps,
//! agent tool access — is a shell-out to the CLI.
//!
//! State ownership:
//! - The CLI owns all its own state under `~/.composio/`. Qaio does
//!   not touch the macOS keychain, does not do OAuth dance, does not
//!   manage tokens, does not touch `~/.claude.json`.
//! - Qaio's only job is: detect install state, surface the right UX
//!   to the user, and dispatch shell commands.
//!
//! Agents spawned by Qaio (`claude` subprocesses) pick up the CLI
//! automatically because `engine/qaio-terminal-manager/src/claude_path.rs`
...
/// Report Qaio's current composio state.
...
pub async fn start_login() -> Result<StartLoginResponse, String> {
    ...
    let result = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || {
            let tmp = std::env::temp_dir().join("qaio-composio-login.json");
...
/// Qaio UI forever — the CLI's own session expiry is 5 minutes, so
/// 330s gives it ~30s of slack before Qaio gives up and returns an
...
/// Log out of Composio. Surfaces CLI errors so callers can toast them.
...
/// app-specific OAuth. Qaio should open this URL with
