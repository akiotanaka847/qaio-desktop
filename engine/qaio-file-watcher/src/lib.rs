...
//!
//! Watches an agent's workspace (`~/Documents/Qaio/<name>/`) and emits
//! `QaioEvent` variants when files change. This catches writes made
//! directly by the CLI agent that bypass Tauri commands — the key piece
//! of Qaio's AI-native reactivity model.
//!
...
/// Map a changed file path to the appropriate `QaioEvent`.
fn classify_change(agent_path: &str, relative: &Path) -> Option<QaioEvent> {
    let rel_str = relative.to_string_lossy();

    // .agents/skills/ (skill.sh convention — source of truth for skills)
    if rel_str.starts_with(".agents/skills") {
        return Some(QaioEvent::SkillsChanged {
    ...
    // .qaio/ internal files — per-type folder layout: .qaio/<type>/<type>.json
    if rel_str.starts_with(".qaio/") {
        let inner = &rel_str[".qaio/".len()..];
        ...
        return match first {
            "activity" => Some(QaioEvent::ActivityChanged { agent_path }),
            "routines" => Some(QaioEvent::RoutinesChanged { agent_path }),
            "routine_runs" => Some(QaioEvent::RoutineRunsChanged { agent_path }),
            "config" => Some(QaioEvent::ConfigChanged { agent_path }),
            "learnings" => Some(QaioEvent::LearningsChanged { agent_path }),
            _ => None,
        };
    }
    ...
    // Non-.qaio files (user documents, etc.)
    Some(QaioEvent::FilesChanged {
        agent_path: agent_path.to_string(),
    })
}

/// Start watching an agent directory for file changes. Emits `QaioEvent`
    ...
    let mut debouncer = new_debouncer(
        ...
        move |events: Result<Vec<notify_debouncer_mini::DebouncedEvent>, notify::Error>| {
            ...
            for event in events {
                ...
                if let Some(qaio_event) = classify_change(&ws_path, relative) {
                    // Use the event type name as dedup key
                    let key = format!("{:?}", std::mem::discriminant(&qaio_event));
                    if emitted.insert(key.clone()) {
                        tracing::debug!("[watcher] emit: {key} for {}", relative.display());
                        sink.emit(qaio_event);
                    }
                }
            }
        },
