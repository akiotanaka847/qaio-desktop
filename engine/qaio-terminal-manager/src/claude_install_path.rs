//! Where Qaio's runtime-installed `claude` binary lives on disk.
//!
//! Lives in `qaio-terminal-manager` (not `qaio-claude-installer`)
...
//! dependency cycle through `qaio-ui-events` (which depends on
//! `qaio-terminal-manager` for `FeedItem`).
//!
