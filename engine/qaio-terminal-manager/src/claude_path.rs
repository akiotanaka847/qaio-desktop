...
/// Qaio's spawned agents can use `composio search`/`composio execute`
...
pub fn init() {
    SHELL_PATH.get_or_init(|| {
        ...
        // first on their PATH.
        for dir in qaio_cli_bundle::bundled_path_entries() {
        ...
        // its directory so the version Qaio pinned + downloaded wins
...
mod tests {
    ...
    fn is_command_available_returns_false_for_garbage() {
        ...
        assert!(!is_command_available(
            "definitely-not-a-real-binary-xyz-zzz-qaio-test"
        ));
    }
