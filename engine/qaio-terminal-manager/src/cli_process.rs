...
pub(crate) async fn run_cli_process(
    ...
    tracing::info!("[qaio:session] {cli_name} process started, reading output");
    ...
    tracing::info!("[qaio:session] stdout closed, waiting for process exit");
    match child.wait().await {
        Ok(status) => {
            tracing::info!("[qaio:session] process exited with {status}");
            ...
            if malformed_provider_json {
                tracing::warn!("[qaio:session] claude failed with malformed provider JSON");
...
fn handle_failed_exit(
    ...
    {
        tracing::warn!("[qaio:session] codex resume failed because rollout was missing");
