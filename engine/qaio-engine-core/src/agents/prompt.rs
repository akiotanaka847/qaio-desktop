...
//! template files.
//!
//! Relocated from `app/qaio-tauri/src/agent.rs` and
...
// ---------------------------------------------------------------------------
// Qaio-flavored seed + system prompt (used by sessions::start*).
// ---------------------------------------------------------------------------

/// Default CLAUDE.md content for a brand-new agent.
pub const DEFAULT_CLAUDE_MD: &str = r#"# Qaio Agent
...
/// Seed the Qaio agent skeleton into an agent directory.
///
/// Creates `CLAUDE.md` (user-editable job description) and the
/// `.qaio/prompts/modes/` directory for per-mode overrides. Does **not**
/// seed any product-layer prompt files — those live in the app process and
/// arrive via the engine's config (e.g. `QAIO_APP_SYSTEM_PROMPT`).
pub fn seed_agent(dir: &Path) -> Result<(), String> {
    ...
    let prompts_dir = dir.join(".qaio/prompts");
    let modes_dir = prompts_dir.join("modes");
    fs::create_dir_all(&modes_dir)
        .map_err(|e| format!("Failed to create .qaio/prompts/modes: {e}"))?;

    if let Err(e) = qaio_agent_files::migrate_agent_data(dir) {
...
/// skills index, integrations list) and nothing about the Qaio product
/// voice. Callers (typically the Qaio app) prepend their own product
...
pub fn build_agent_context(
    ...
    parts.push(format!(
         ...
         **CRITICAL RULES:**\n\
         ...
         - Qaio data goes in `.qaio/` (relative to this directory).\n\
    ...
    let integrations_path = dir.join(".qaio/integrations.json");
...
mod tests {
    ...
    fn build_agent_context_includes_learnings_snapshot() {
        let d = TempDir::new().unwrap();
        let learnings_dir = d.path().join(".qaio/learnings");
