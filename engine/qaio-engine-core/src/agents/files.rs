...
//!    operate on `.qaio/<type>/<type>.json` and friends, with path-traversal
//!    safety enforced by `qaio-agent-files`. Writes return the matching
//!    `QaioEvent` so the caller (REST handler or Tauri command) can fan it
...
//!    agent's CLAUDE.md (not under `.qaio/`).
//!
//! Relocated from `app/qaio-tauri/src/agent_files.rs` and the FS-ops chunk
//! of `app/qaio-tauri/src/agent_commands.rs` as part of the engine-standalone
...
/// `QaioEvent` that the caller should emit (or `None` for paths outside
/// the typed `.qaio/` layout).
pub fn write_agent_file(
    agent_root: &Path,
    agent_path: &str,
    rel_path: &str,
    content: &str,
) -> CoreResult<Option<QaioEvent>> {
    files::write_file_atomic(agent_root, rel_path, content).map_err(file_err)?;
    Ok(event_for_write(agent_path, rel_path))
}

/// Seed the embedded JSON Schemas into `.qaio/<type>/<type>.schema.json`.
pub fn seed_agent_schemas(agent_root: &Path) -> CoreResult<()> {
    files::seed_schemas(agent_root).map_err(file_err)
}

/// Idempotent migration from the legacy flat `.qaio/` layout.
pub fn migrate_agent_files(agent_root: &Path) -> CoreResult<()> {
    files::migrate_agent_data(agent_root).map_err(file_err)
}

fn event_for_write(agent_path: &str, rel_path: &str) -> Option<QaioEvent> {
    if rel_path == "CLAUDE.md" || rel_path.starts_with(".qaio/prompts/") {
        return Some(QaioEvent::ContextChanged {
    ...
    Some(match kind.as_str() {
        "activity" => QaioEvent::ActivityChanged { agent_path },
        "routines" => QaioEvent::RoutinesChanged { agent_path },
        "routine_runs" => QaioEvent::RoutineRunsChanged { agent_path },
        "config" => QaioEvent::ConfigChanged { agent_path },
        "learnings" => QaioEvent::LearningsChanged { agent_path },
        _ => return None,
    })
}
...
mod tests {
    ...
    fn read_missing_returns_empty() {
        let d = tmp();
        let s = read_agent_file(d.path(), ".qaio/activity/activity.json").unwrap();
        assert_eq!(s, "");
    }
    ...
    fn write_and_read_round_trip() {
        ...
        .unwrap();
        assert!(matches!(ev, Some(QaioEvent::ActivityChanged { .. })));
        let s = read_agent_file(d.path(), ".qaio/activity/activity.json").unwrap();
        assert_eq!(s, "[]");
    }
    ...
    fn write_claude_md_emits_context_changed() {
        ...
        assert!(matches!(ev, Some(QaioEvent::ContextChanged { .. })));
    }
    ...
    fn list_project_files_skips_dotdirs_and_unknown_ext() {
        ...
        std::fs::create_dir_all(d.path().join(".qaio/activity")).unwrap();
        std::fs::write(d.path().join(".qaio/activity/activity.json"), "[]").unwrap();
