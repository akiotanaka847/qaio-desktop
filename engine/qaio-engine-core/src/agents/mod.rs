//! Typed read/write operations for `.qaio/` agent files.
//!
//! Every Qaio agent project stores agent-visible data in a `.qaio/` folder
...
//!
//! Relocated from `app/qaio-tauri/src/agent_store/` as part of the engine
...
//! (`qaio_engine_core::conversations`) and is already REST-exposed under
...
/// File-backed store for `.qaio/` agent data.
///
...
impl AgentStore {
    ...
    pub fn ensure_qaio_dir(&self) -> CoreResult<()> {
        store::ensure_qaio_dir(&self.root)
    }
