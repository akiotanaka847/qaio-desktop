...
//!
//! Reads installed agent manifests from `<home>/agents/*/qaio.json`.
...
///
/// Each subdirectory with a valid `qaio.json` contributes one entry.
...
pub fn list_installed(home: &Path) -> CoreResult<Vec<InstalledConfig>> {
    ...
    for entry in fs::read_dir(&dir)?.flatten() {
        ...
        let config_path = path.join("qaio.json");
...
mod tests {
    ...
    fn finds_valid_configs_skips_broken() {
        ...
        fs::write(broken.join("qaio.json"), "{not json").unwrap();
