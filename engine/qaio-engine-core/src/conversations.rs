//! Conversation listing — derived view over `.qaio/activity/activity.json`.
//!
//! Every conversation is an activity with a UUID-scoped session key
//! (`activity-{id}`). This module reads the JSON file directly via
//! `qaio_agent_files` so it does not depend on the full agent_store
//! layer. Phase 2 slice 4 will move the activity writer here too and
//! collapse the duplication with `app/qaio-tauri/src/agent_store/`.
...
fn read_activities(root: &Path) -> CoreResult<Vec<ActivityRow>> {
    let contents = files::read_file(root, ".qaio/activity/activity.json")
...
mod tests {
    ...
    fn seed(dir: &Path, activities: serde_json::Value) {
        let rel = dir.join(".qaio").join("activity");
