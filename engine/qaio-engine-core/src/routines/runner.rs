...
use crate::routines::{self, ensure_qaio_dir};
...
/// real impl on top of `qaio-agents-conversations::session_runner`; tests
...
pub async fn run_routine(
    ...
    ensure_qaio_dir(&working_dir)?;
    let run = routine_runs::create(&working_dir, routine_id)?;
    events.emit(QaioEvent::RoutineRunsChanged {
    ...
    if is_silent {
        ...
        events.emit(QaioEvent::CompletionToast {
...
/// `qaio_tauri::paths::expand_tilde` so the runner stays in the engine tree.
