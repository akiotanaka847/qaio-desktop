...
pub async fn create_worktree(req: CreateWorktreeRequest) -> CoreResult<WorktreeInfo> {
    ...
    let branch_name = req.branch.unwrap_or_else(|| format!("qaio/{}", req.name));
...
pub async fn run_shell(req: RunShellRequest) -> CoreResult<String> {
    ...
    let output = Command::new("sh")
        ...
        .env(
            "PATH",
            qaio_terminal_manager::claude_path::shell_path(),
        )
        .output()
        .await
...
mod tests {
    ...
    async fn create_list_remove_worktree() {
        ...
        .await
        .unwrap();
        assert_eq!(info.branch, "qaio/feature-x");
        assert!(!info.is_main);
        ...
        assert!(list.iter().any(|w| w.branch == "qaio/feature-x"));
