//! Qaio Store — relocated from `app/src-tauri/src/commands/store.rs`.
//!
...
const STORE_API_DEFAULT: &str = "https://store.getqaio.ai/api";

fn store_api() -> String {
    std::env::var("QAIO_STORE_API").unwrap_or_else(|_| STORE_API_DEFAULT.to_string())
}
...
pub async fn install_agent(agents_dir: &Path, req: InstallAgent) -> CoreResult<()> {
    if let Some(agent_id) = req.repo.strip_prefix("qaio-store/") {
...
pub async fn install_agent_from_github(agents_dir: &Path, github_url: &str) -> CoreResult<String> {
    ...
    let config_bytes: Vec<u8> = fetch_github_raw(&owner, &repo, "qaio.json")
        .await?
        .ok_or_else(|| CoreError::NotFound(format!("no qaio.json in {owner}/{repo}")))?;

    let config: serde_json::Value = serde_json::from_slice(&config_bytes)?;
    let agent_id = config["id"]
        .as_str()
        .ok_or_else(|| CoreError::BadRequest("qaio.json missing 'id' field".into()))?
        .to_string();
...
pub async fn check_agent_updates(agents_dir: &Path) -> CoreResult<Vec<String>> {
    ...
    for entry in entries.flatten() {
        ...
        if source["source"].as_str() == Some("qaio-store") {
        ...
        let local_config = fs::read_to_string(path.join("qaio.json")).unwrap_or_default();
        let remote_config = match fetch_github_raw(owner, repo_name, "qaio.json").await {
        ...
        if config_changed {
            let _ = fs::write(path.join("qaio.json"), &remote_config);
        }
...
pub async fn install_workspace_from_github(
    ...
    for agent_folder in &template.agents {
        ...
        let config_path = format!("{prefix}/qaio.json");

        let config_bytes = match fetch_github_raw(&owner, &repo, &config_path).await {
            Ok(Some(bytes)) => bytes,
            _ => {
                tracing::warn!("[workspace-import] skipping {agent_folder}: no qaio.json");
                continue;
            }
        };
        ...
        fs::write(def_dir.join("qaio.json"), &config_bytes)?;
        ...
        fs::create_dir_all(agent_folder_path.join(".qaio"))?;
        ...
        fs::write(
            agent_folder_path.join(".qaio/agent.json"),
        ...
        let qaio = agent_folder_path.join(".qaio");
        if !qaio.join("activity.json").exists() {
            let _ = fs::write(qaio.join("activity.json"), "[]");
        }
        if !qaio.join("config.json").exists() {
            let _ = fs::write(qaio.join("config.json"), "{}");
        }

        let _ = qaio_agent_files::migrate_agent_data(&agent_folder_path);
...
/// Default agents dir: `$HOME/.qaio/agents`. Override via `QAIO_AGENTS_DIR`.
pub fn default_agents_dir() -> PathBuf {
    if let Ok(p) = std::env::var("QAIO_AGENTS_DIR") {
...
mod tests {
    ...
    fn parse_shorthand() {
        assert_eq!(
            parse_github_ref("getqaio/qaio-store").unwrap(),
            ("getqaio".into(), "qaio-store".into())
        );
    }
    ...
    fn parse_full_url() {
        assert_eq!(
            parse_github_ref("https://github.com/getqaio/qaio-store").unwrap(),
            ("getqaio".into(), "qaio-store".into())
        );
        assert_eq!(
            parse_github_ref("https://github.com/getqaio/qaio-store/").unwrap(),
        ...
        assert_eq!(
            parse_github_ref("https://github.com/getqaio/qaio-store/tree/main").unwrap(),
            ("getqaio".into(), "qaio-store".into())
        );
    }
    ...
    fn uninstall_removes_dir() {
        ...
        fs::write(a.join("qaio.json"), "{}").unwrap();
        assert!(a.exists());
