...
use qaio_terminal_manager::claude_path;
use std::path::PathBuf;

pub(super) fn resolve_claude() -> (InstallSource, Option<PathBuf>) {
    if qaio_claude_installer::is_installed() {
        return (
            InstallSource::Managed,
            Some(qaio_claude_installer::cli_path()),
        );
    }
...
pub(super) fn resolve_codex() -> (InstallSource, Option<PathBuf>) {
    if let Some(path) = qaio_cli_bundle::bundled_codex_path() {
