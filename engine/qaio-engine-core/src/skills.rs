...
//! `QaioEvent::SkillsChanged` stream.
...
impl From<SkillError> for CoreError {
    fn from(err: SkillError) -> Self {
        use qaio_engine_protocol::ErrorCode;
        match err {
...
fn emit_skills_changed(events: &DynEventSink, workspace_path: &str) {
    events.emit(QaioEvent::SkillsChanged {
...
pub fn list(workspace_path: &str) -> CoreResult<Vec<SkillSummaryResponse>> {
    ...
    let summaries = qaio_skills::list_skills(&dir)?;
    for s in &summaries {
        ...
        .map(|s| SkillSummaryResponse {
            ...
            inputs: s
                ...
                .map(|i| SkillInputResponse {
                    ...
                    kind: match i.kind {
                        qaio_skills::SkillInputKind::Text => "text".into(),
                        qaio_skills::SkillInputKind::Textarea => "textarea".into(),
                        qaio_skills::SkillInputKind::Select => "select".into(),
                    },
                    required: i.required,
                    default: i.default,
                    options: i.options,
...
pub fn load(workspace_path: &str, name: &str) -> CoreResult<SkillDetailResponse> {
    let dir = skills_dir(workspace_path);
    let skill = qaio_skills::load_skill(&dir, name)?;
...
pub fn delete(events: &DynEventSink, workspace_path: &str, name: &str) -> CoreResult<()> {
    let dir = skills_dir(workspace_path);
    qaio_skills::delete_skill(&dir, name)?;
...
pub fn save(events: &DynEventSink, name: &str, req: SaveSkillRequest) -> CoreResult<()> {
    let dir = skills_dir(&req.workspace_path);
    qaio_skills::edit_skill(&dir, name, &req.content)?;
    emit_skills_changed(events, &req.workspace_path);
    Ok(())
}

pub async fn list_from_repo(source: &str) -> CoreResult<Vec<RepoSkill>> {
    qaio_skills::remote::list_skills_from_repo(source)
        .await
        .map_err(Into::into)
}
...
pub async fn install_from_repo(
    ...
    let repo_skills: Vec<RepoSkill> = req
        ...
        .collect();
    let names = qaio_skills::remote::install_from_repo(&dir, &req.source, &repo_skills).await?;
    for n in &names {
...
pub async fn search_community(query: &str) -> CoreResult<Vec<CommunitySkill>> {
    qaio_skills::remote::search_skills(query)
        .await
        .map_err(Into::into)
}
...
pub async fn popular_community() -> CoreResult<Vec<CommunitySkill>> {
    qaio_skills::remote::fetch_popular_skills()
        .await
        .map_err(Into::into)
}
...
pub async fn install_community(
    ...
    let dir = skills_dir(&req.workspace_path);
    let name = qaio_skills::remote::install_skill(&dir, &req.source, &req.skill_id).await?;
...
mod tests {
    use super::*;
    use qaio_ui_events::{BroadcastEventSink, DynEventSink};
    use std::sync::Arc;
    ...
    async fn create_then_list_and_load() {
        ...
        matches!(ev, QaioEvent::SkillsChanged { .. });
    ...
    fn create_duplicate_conflicts() {
        ...
        .unwrap_err();
        assert_eq!(err.code(), qaio_engine_protocol::ErrorCode::Conflict);
    ...
    fn load_missing_is_not_found() {
        ...
        assert_eq!(err.code(), qaio_engine_protocol::ErrorCode::NotFound);
