...
//!
//! `sessions::cancel` needs a real guarantee: after Stop, the
//! claude/codex/gemini/kimi process AND everything it spawned (shell
//! tools, MCP servers, subagent helpers) are actually gone. Sending one
//! SIGTERM and hoping is not enough — Node CLIs trap TERM, children can
//! ignore it, and `kill`'s exit status only proves the signal was *sent*,
//! not that the process died.
//!
//! Platform strategies live in the submodules: [`unix`] (TERM the process
//! group + descendant snapshot, poll, escalate to KILL) and [`windows`]
//! (`taskkill /T /F`, verify via `tasklist`, retry).
