...
//!
//! Each Qaio "session" is one running Claude/Codex CLI subprocess with a
...
//!   `qaio-agents-conversations`, streaming updates to the engine's event
...
use qaio_agents_conversations::session_id_tracker::SessionIdTracker;
use qaio_agents_conversations::session_pids::SessionPidMap;
use qaio_agents_conversations::session_runner::{self, PersistOptions};
use qaio_db::Database;
use qaio_terminal_manager::{FeedItem, Provider};
    ...
    tokio::spawn({
        ...
        async move {
            ...
            .await
            {
                ...
                events.emit(QaioEvent::SessionStatus {
                    agent_path,
                    session_key,
                    status: "error".into(),
...
async fn run_start(
    ...
    //   embedding app (Qaio desktop) passed in via `QAIO_APP_SYSTEM_PROMPT`.
    ...
    match crate::agents::activity::set_status_by_session_key(&agent_dir, &session_key, "running") {
        Ok(Some(_)) => {
            events.emit(QaioEvent::ActivityChanged {
            ...
            match file_changes::snapshot(&working_dir_for_end) {
                Ok(after) => {
                    ...
                    if !changes.is_empty() {
                        ...
                        events_for_end.emit(QaioEvent::FeedItem {
                            agent_path: agent_path_for_end.clone(),
                            session_key: session_key_for_end.clone(),
                            item,
                        });
                        events_for_end.emit(QaioEvent::FilesChanged {
                            agent_path: agent_path_for_end.clone(),
                        });
    ...
    match crate::agents::activity::set_status_by_session_key(
        ...
        Ok(Some(_)) => {
            ...
            events_for_end.emit(QaioEvent::ActivityChanged {
...
pub async fn cancel(
    ...
    events.emit(QaioEvent::FeedItem {
...
mod tests {
    use super::*;
    use qaio_db::Database;
    use qaio_ui_events::NoopEventSink;
    use std::sync::Arc;
