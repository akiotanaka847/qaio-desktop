...
/// `<workspace-agent>/.qaio/bundled-package.json`. Reading it lets
/// the next sync pass know which migration steps still need applying.
const BUNDLED_PACKAGE_MARKER: &str = ".qaio/bundled-package.json";
...
/// we use in `qaio.json` / `catalog.json`. Treats missing trailing
...
fn bundled_store_root() -> Option<PathBuf> {
    if let Ok(dir) = std::env::var("QAIO_STORE_DIR") {
...
pub(super) fn install_bundled_agent(
    ...
    let root = bundled_store_root()
        .ok_or_else(|| CoreError::NotFound("bundled Qaio Store not found".into()))?;
    let source_dir = root.join("agents").join(source_agent_id);
    if !source_dir.join("qaio.json").exists() {
...
pub(super) fn sync_bundled_agent_instances(
    ...
    for workspace in workspaces::read_all(docs_dir)? {
        ...
        for entry in fs::read_dir(&workspace_dir)?.flatten() {
            ...
            let meta_path = folder.join(".qaio").join("agent.json");
            if !meta_path.exists() {
                continue;
            }
...
fn clear_seeded_intro_activity(agent_dir: &Path) -> CoreResult<bool> {
    ...
    for relative in [".qaio/activity.json", ".qaio/activity/activity.json"] {
...
fn sync_existing_skill_metadata(source_dir: &Path, target_dir: &Path) -> CoreResult<bool> {
    ...
    let (mut source_summary, _) = match qaio_skills::format::parse_file(&source_md) {
        Ok(parsed) => parsed,
        Err(e) => {
            tracing::warn!(
    ...
    let (target_summary, target_body) = match qaio_skills::format::parse_content(&target_raw) {
    ...
    let updated = qaio_skills::format::serialize(&source_summary, &target_body);
...
mod tests {
  ...
  "agents": [{{
    ...
    fn bundled_catalog_reads_from_store_dir() {
        ...
        std::env::set_var("QAIO_STORE_DIR", store.path());

        let catalog = bundled_catalog().unwrap().unwrap();

        std::env::remove_var("QAIO_STORE_DIR");
        assert_eq!(catalog.len(), 1);
        assert_eq!(catalog[0].repo, "qaio-store/demo");
...
"#,
        ...
        let (summary, body) = qaio_skills::format::parse_content(&updated).unwrap();
    ...
    fn sync_bundled_agent_instances_removes_seeded_intro_activity_only() {
        ...
        let legacy_activity = agent_dir.join(".qaio/activity.json");
        let nested_activity = agent_dir.join(".qaio/activity/activity.json");
    ...
    fn bundled_store_skills_parse_without_forms() {
        ...
        for file in &files {
            let (summary, _) = qaio_skills::format::parse_file(file)
    ...
    fn bundled_store_agents_do_not_seed_activity_cards() {
        ...
        for entry in entries.flatten() {
            let manifest_path = entry.path().join("qaio.json");
            ...
            assert!(
                !seeds.contains_key(".qaio/activity.json"),
                "{} must not seed legacy activity cards",
                manifest_path.display()
            );
            assert!(
                !seeds.contains_key(".qaio/activity/activity.json"),
  ...
  {"from": "0.1.4", "to": "0.2.0", "renames": {"old-slug": "new-slug"}}
        ...
        // rename step.
        let marker_body =
            fs::read_to_string(docs.path().join("Acme/Ops/.qaio/bundled-package.json"))
