...
pub struct SkillSummary {
    ...
    /// user-authored skills survive round-trips, but current Qaio UX
...
pub fn list_skills(skills_dir: &Path) -> Result<Vec<SkillSummary>, SkillError> {
    ...
    for entry in entries.flatten() {
        ...
        match format::parse_file(&skill_md) {
            ...
            Err(e) => tracing::warn!("[qaio-skills] skipping {}: {e}", path.display()),
        }
    }
...
/// `.agents/skills/` and have Qaio migrate it on the next list call.
fn migrate_flat_files(skills_dir: &Path) -> Result<(), SkillError> {
    ...
    for entry in entries.flatten() {
        ...
        if target_dir.exists() {
            ...
            tracing::warn!(
                "[qaio-skills] skipping migration of {}: target {} exists",
                path.display(),
                target_dir.display()
            );
            continue;
        ...
        tracing::info!(
            "[qaio-skills] migrated flat skill {} -> {}",
            path.display(),
            target.display()
        );
    }
