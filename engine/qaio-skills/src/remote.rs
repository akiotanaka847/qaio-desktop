        ...
        match search_skills_at(client, endpoint, query, retry_delay).await {
            ...
            Err(err) => {
                ...
                if let Some(skills) = stale {
                    tracing::warn!(
                        "[qaio-skills] community search failed, returning cached results: {err}"
                    );
                    Ok(skills)
                } else {
                    Err(err)
    ...
    async fn popular(
        ...
        match search_skills_at(client, endpoint, seed, retry_delay).await {
            ...
            Err(err) => {
                ...
                if let Some(skills) = stale {
                    tracing::warn!(
                        "[qaio-skills] popular feed fetch failed, returning cached results: {err}"
                    );
                    Ok(truncate(skills, limit))
                } else {
                    Err(err)
...
pub async fn list_skills_from_repo(source: &str) -> Result<Vec<RepoSkill>, SkillError> {
    ...
    if tree.truncated {
        tracing::warn!(
            "[qaio-skills] repo tree for {source} was truncated — some skills may be missing"
        );
    }
