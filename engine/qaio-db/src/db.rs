...
impl Database {
    ...
    pub async fn connect(path: &std::path::Path) -> Result<Self> {
        ...
        tracing::info!("[qaio:db] using local SQLite at {}", path.display());
...
/// Returns the Qaio data root.
///
/// - Release builds: `~/.qaio/` — what real users see.
/// - Debug builds (`pnpm tauri dev`, `cargo run`): `~/.dev-qaio/` — keeps
///   development state isolated so you can install a real release of Qaio
///   alongside without clobbering each other.
///
/// Override via `QAIO_HOME` env var (wins regardless of build profile).
/// Delete this directory for a completely clean slate.
pub fn qaio_dir() -> PathBuf {
    if let Ok(override_path) = std::env::var("QAIO_HOME") {
