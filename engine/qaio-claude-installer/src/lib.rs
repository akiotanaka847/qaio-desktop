...
//! permit redistribution inside Qaio's bundle. So we do the next
...
//!      `qaio-cli-bundle`). The upstream install script chases
//!      "latest", which would silently roll a new version on every
//!      Qaio install and break the version lockstep we use to
...
//!   4. **Progress events** — the installer emits `QaioEvent`s so
...
//!    `QaioEvent::ClaudeCliInstalling { progress_pct }` every ~250 ms.
...
// Path-resolution functions live in `qaio-terminal-manager` so the
...
// `provider/resolve.rs`).
pub use qaio_terminal_manager::claude_install_path::{
...
pub async fn ensure_and_upgrade(sink: DynEventSink, db: Database) {
    ...
    let Some(manifest) = resolve_manifest() else {
        ...
        sink.emit(QaioEvent::ClaudeCliReady);
        return;
    };
    ...
    sink.emit(QaioEvent::ClaudeCliInstalling { progress_pct: 0 });

    let sink_for_progress = sink.clone();
    let result = install(&entry, move |pct| {
        sink_for_progress.emit(QaioEvent::ClaudeCliInstalling { progress_pct: pct });
    })
    .await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            sink.emit(QaioEvent::ClaudeCliFailed { message: e });
        }
    }
}
...
/// running `cargo run -p qaio-engine-server` against an unbundled
/// build still get auto-install.
fn resolve_manifest() -> Option<qaio_cli_bundle::CliDepsManifest> {
    if let Some(m) = qaio_cli_bundle::load_bundled_manifest() {
        return Some(m);
    }
    ...
    loop {
        ...
        if candidate.is_file() {
            return qaio_cli_bundle::CliDepsManifest::load(&candidate).ok();
        }
        match here.parent() {
            Some(p) => here = p,
            None => return None,
...
pub async fn install(
    entry: &qaio_cli_bundle::CliEntry,
...
pub async fn install_to(
    ...
    let platform = qaio_cli_bundle::host_platform_key();
    let url = entry
        .url_for(platform)
...
mod tests {
    ...
    fn entry_for(server_uri: &str, payload: &[u8]) -> qaio_cli_bundle::CliEntry {
        ...
        let manifest = serde_json::json!({
            "claude-code": {
                ...
                "license": "PROPRIETARY",
                "urls": {
                    qaio_cli_bundle::host_platform_key(): format!("{server_uri}/claude")
                },
                "checksums": {
                    qaio_cli_bundle::host_platform_key(): actual
                }
            }
        });
        ...
        let m = qaio_cli_bundle::CliDepsManifest::load(tmp.path()).unwrap();
    ...
    async fn install_rejects_checksum_mismatch_and_cleans_temp() {
        ...
        let manifest = serde_json::json!({
            "claude-code": {
                ...
                "binary_name": "claude",
                "urls": {
                    qaio_cli_bundle::host_platform_key(): format!("{}/claude", server.uri())
                },
                "checksums": {
                    qaio_cli_bundle::host_platform_key():
        ...
        let entry = qaio_cli_bundle::CliDepsManifest::load(tmp.path())
            .unwrap()
            .entry("claude-code")
            .unwrap();
