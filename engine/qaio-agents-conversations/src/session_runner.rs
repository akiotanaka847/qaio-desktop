...
use qaio_terminal_manager::provider_auth::{
    probe_claude_auth_status, probe_gemini_auth_status, ProviderAuthState,
};
...
async fn provider_auth_state(provider: Provider) -> ProviderAuthState {
    match provider {
        ...
        Provider::Gemini => probe_gemini_auth_status().await,
    }
}
        ...
        while let Some(update) = rx.recv().await {
            match update {
                            ...
                            AuthFeedAction::VerifyProviderStatus => {
                                ...
                                if auth_state != ProviderAuthState::Unauthenticated {
                                    ...
                                    if !sent_auth_required {
                                        ...
                                        sink.emit(QaioEvent::AuthRequired {
                ...
                SessionUpdate::Status(ref status) => {
                    ...
                    sink.emit(QaioEvent::SessionStatus {
                        agent_path: agent_path_for_events.clone(),
                        session_key: key.clone(),
                        status: status_str,
                        error: err,
...
mod tests {
    ...
    fn serializes_tool_runtime_error_for_history() {
        let item = FeedItem::ToolRuntimeError {
            kind: qaio_terminal_manager::ToolRuntimeErrorKind::LocalTool,
