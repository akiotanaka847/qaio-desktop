...
//!
//! Current IDs are provider-scoped under `.qaio/sessions/{provider}/`.
//! The legacy flat `.qaio/sessions/{session_key}.sid` path is still read as a
...
const PROVIDERS: [Provider; 3] = [Provider::Anthropic, Provider::OpenAI, Provider::Gemini];

fn sessions_dir(agent_dir: &Path) -> PathBuf {
    agent_dir.join(".qaio").join("sessions")
}

/// Return the legacy flat path where older Qaio versions persisted a resume ID.
...
impl SessionIdTracker {
    ...
    /// `.qaio/sessions/{provider}/{session_key}.sid` is stored.
...
mod tests {
    ...
    fn provider_paths_are_distinct() {
        ...
        assert!(anthropic.ends_with(".qaio/sessions/anthropic/chat.sid"));
        assert!(openai.ends_with(".qaio/sessions/openai/chat.sid"));
    }
