//! qaio-agent-files — generic file-level access to an agent's `.qaio/` directory.
//!
//! Each data "type" lives in its own folder with a co-located JSON Schema:
//!   .qaio/<type>/<type>.json
//!   .qaio/<type>/<type>.schema.json
//!
...
/// Returns the first path component of `.qaio/<type>/...` — e.g. "activity".
pub fn classify(rel: &str) -> Option<String> {
    let p = Path::new(rel);
    let mut it = p.components();
    // Expect first component to be ".qaio"
    let first = it.next()?;
    let s = match first {
...
/// Seed the five embedded JSON Schemas under `.qaio/<type>/<type>.schema.json`.
...
pub fn seed_schemas(agent_root: &Path) -> Result<()> {
    for (name, content) in schemas::ALL {
        let rel = format!(".qaio/{name}/{name}.schema.json");
...
///   .qaio/activity.json
///   .qaio/routines.json
///   .qaio/routine_runs.json
///   .qaio/config.json
///   .qaio/memory/learnings.md
///
/// New:
///   .qaio/activity/activity.json
///   .qaio/routines/routines.json
///   .qaio/routine_runs/routine_runs.json
///   .qaio/config/config.json
///   .qaio/learnings/learnings.json
///
...
pub fn migrate_agent_data(agent_root: &Path) -> Result<()> {
    ...
    for name in ["activity", "routines", "routine_runs", "config"] {
        let old_rel = format!(".qaio/{name}.json");
        let new_rel = format!(".qaio/{name}/{name}.json");
    ...
    let learnings_md = agent_root.join(".qaio/memory/learnings.md");
    let learnings_new = agent_root.join(".qaio/learnings/learnings.json");
    if learnings_md.exists() && !learnings_new.exists() {
        ...
        write_file_atomic(agent_root, ".qaio/learnings/learnings.json", &body)?;
    ...
    // `.qaio/prompts/`. These were never user-editable through any UI —
    // the Qaio product prompt lives in the app process now. Deleting is
    // safe: no real user edits to preserve. User's mode overrides in
    // `modes/` are untouched.
    for legacy in [
        ".qaio/prompts/system.md",
        ".qaio/prompts/self-improvement.md",
    ] {
...
mod tests {
    ...
    fn roundtrip_write_read() {
        ...
        write_file_atomic(dir.path(), ".qaio/activity/activity.json", "[]").unwrap();
        let got = read_file(dir.path(), ".qaio/activity/activity.json").unwrap();
        assert_eq!(got, "[]");
    }
    ...
    fn classify_activity() {
        assert_eq!(
            classify(".qaio/activity/activity.json"),
            Some("activity".to_string())
        );
        assert_eq!(classify(".qaio/routines/routines.json"), Some("routines".to_string()));
    ...
    fn seed_schemas_writes_all() {
        ...
        for (name, _) in schemas::ALL {
            assert!(dir.path().join(format!(".qaio/{name}/{name}.schema.json")).exists());
        }
    }
    ...
    fn migrate_moves_legacy_files() {
        let dir = TempDir::new().unwrap();
        let legacy = dir.path().join(".qaio/activity.json");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, "[{\"id\":\"a\"}]").unwrap();

        migrate_agent_data(dir.path()).unwrap();

        let new = dir.path().join(".qaio/activity/activity.json");
        assert!(new.exists());
    ...
    fn migrate_removes_legacy_product_prompts() {
        let dir = TempDir::new().unwrap();
        let prompts = dir.path().join(".qaio/prompts");
    ...
    fn migrate_learnings_md_to_json() {
        let dir = TempDir::new().unwrap();
        let md = dir.path().join(".qaio/memory/learnings.md");
        fs::create_dir_all(md.parent().unwrap()).unwrap();
        fs::write(&md, "- first learning\n- second learning\n").unwrap();

        migrate_agent_data(dir.path()).unwrap();

        let json = fs::read_to_string(dir.path().join(".qaio/learnings/learnings.json")).unwrap();
