//! Outbound tunnel connection to the Qaio relay.
//!
...
pub struct TunnelConfig {
    /// Qaio home directory — holds `tunnel.json` so the client can
    /// persist / invalidate / re-allocate identity without bouncing the
    /// engine.
    pub home_dir: std::path::PathBuf,
    /// Relay base URL, e.g. `https://tunnel.getqaio.ai`. The client
...
impl TunnelClient {
    ...
    pub async fn run(self) {
        ...
        loop {
            ...
            match run_result {
                Ok(()) => {
                    tracing::info!(target: "qaio_tunnel", "tunnel closed cleanly, reconnecting");
                    backoff_ms = 500;
