...
impl TunnelClient {
    pub(super) async fn run_once(&self) -> Result<(), RunError> {
        ...
        tracing::info!(target: "qaio_tunnel", "tunnel connected");
        ...
        let writer = tokio::spawn(async move {
            while let Some(frame) = out_rx.recv().await {
                let text = match serde_json::to_string(&frame) {
                    Ok(s) => s,
                    Err(e) => {
                        tracing::warn!(target: "qaio_tunnel", error = %e, "serialize frame");
                        continue;
                    }
                };
                if let Err(e) = sink.send(WsMsg::Text(text.into())).await {
                    tracing::debug!(target: "qaio_tunnel", error = %e, "writer closed");
                    break;
                }
            }
        });
        ...
        if let Err(e) = writer.await {
            tracing::debug!(target: "qaio_tunnel", error = %e, "writer task join failed");
        }
        outcome
    }
}
