//! Resolve bundled CLI binaries that ship inside the Qaio `.app`.
//!
//! Qaio bundles two upstream CLIs into the signed/notarized desktop app
...
//! `qaio-claude-installer` downloads it on first launch using URLs +
...
//! builds (`pnpm tauri dev`, `cargo run -p qaio-engine-server`)
...
//!   Contents/
//!     MacOS/
//!       qaio-app                ← parent app binary
//!       qaio-engine[-<triple>]  ← engine sidecar (current_exe here)
//!     Resources/
...
//! Windows MSI / NSIS:
//! ```text
//! C:\Program Files\Qaio\
//!   qaio-app.exe
//!   qaio-engine.exe                ← current_exe here
//!   resources\
//!     bin\                            ← bundled_bin_dir() returns this
//!       ...
//! ```
//!
//! Linux AppImage / deb / rpm: not yet supported by Qaio releases.
...
//
// Reading the manifest lets callers (notably qaio-claude-installer)
...
// rest; using compile-time `include_str!` would force a Qaio rebuild
...
/// stay readable without a Qaio rebuild.
#[derive(Debug, Clone)]
...
/// Resources/ tree like macOS). Both `qaio-app.exe` and the
/// `qaio-engine.exe` sidecar live at the install root, so the
...
mod tests {
    ...
    fn rejects_random_path() {
        let tmp = tempfile::tempdir().unwrap();
        let exe = tmp.path().join("random").join("qaio-engine");
    ...
    fn detects_windows_sibling_layout() {
        ...
        let exe = exe_dir.join("qaio-engine");
        fs::write(&exe, b"").unwrap();
        let bin = exe_dir.join("resources").join(BIN_SUBDIR);
        fs::create_dir_all(&bin).unwrap();
    ...
    fn detects_windows_msi_install_layout() {
        ...
        // install root next to qaio-app.exe.
        let tmp = tempfile::tempdir().unwrap();
        let install = tmp.path().join("Qaio");
        fs::create_dir_all(&install).unwrap();
        let exe = install.join("qaio-engine.exe");
        fs::write(&exe, b"").unwrap();
