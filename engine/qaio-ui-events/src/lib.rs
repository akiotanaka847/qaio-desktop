...
//!
//! This crate is transport-neutral. Producers construct `QaioEvent`
...
pub enum QaioEvent {
    ...
    /// Routines list changed (.qaio/routines.json).
    RoutinesChanged {
        agent_path: String,
    },
    /// Routine runs changed (.qaio/routine_runs.json).
    RoutineRunsChanged {
        agent_path: String,
    },
    ...
    /// Activity list changed (.qaio/activity.json).
    ActivityChanged {
        agent_path: String,
    },
    ...
    /// Agent files changed (non-.qaio files).
    FilesChanged {
        agent_path: String,
    },
    /// Config changed (.qaio/config.json).
    ConfigChanged {
        agent_path: String,
    },
    /// Context files changed (CLAUDE.md, .qaio/prompts/).
    ContextChanged {
        agent_path: String,
    },
    ...
    /// Learnings changed (.qaio/learnings/learnings.json).
    LearningsChanged {
        agent_path: String,
    },
    ...
    /// watch by `qaio_composio::connection_watcher`. Frontend
    ...
    // downloads it on first launch via `qaio-claude-installer`. The
...
impl BroadcastEventSink {
    ...
    pub fn subscribe(&self) -> broadcast::Receiver<QaioEvent> {
        self.tx.subscribe()
    }
...
mod tests {
    ...
    async fn broadcast_sink_delivers_to_subscribers() {
        ...
        matches!(e1, QaioEvent::Toast { .. });
        matches!(e2, QaioEvent::Toast { .. });
    }
