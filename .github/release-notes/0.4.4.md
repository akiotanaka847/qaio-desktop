...
Qaio should detect 0.4.4, download it, install it, and relaunch into the new version on its own. You will see the sidebar chip change shape, that confirms the auto-update worked end-to-end.

### If you are on 0.4.2 or earlier

Auto-update will not pull you forward. Quit Qaio, drag a fresh DMG over `/Applications/Qaio.app`, and reopen. After that one manual step you are on the healthy updater path and 0.4.5 onward will apply automatically.

### Before you upgrade

- Quit Qaio before installing. macOS does not always replace a running app. If in doubt: `rm -rf /Applications/Qaio.app` and drag the new copy in.
