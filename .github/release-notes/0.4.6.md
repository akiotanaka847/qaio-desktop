...
- **Qaio for Windows** ships as a signed-by-minisign MSI installer alongside the Mac DMG. Same agent, same engine, same chat surface. Composio, codex, and claude-code all run as native Windows executables. No WSL, no Docker, no terminal anywhere in the install or first-run path.
- **In-app auto-update on Windows.** Once you're on a Qaio for Windows build, future versions arrive the same way they do on Mac: a notification in the app, one click to install, automatic relaunch.
- **Windows first-run hardening.** Sign-in opens the OAuth URL through `cmd /C start` instead of macOS `open` (Windows users could not get past the sign-in screen before). The engine spawns with `CREATE_NEW_PROCESS_GROUP` so the parent app's console events stop killing the subprocess every couple of minutes (`STATUS_CONTROL_C_EXIT`). The bundled CLI resolver knows the MSI install layout, so claude-code and composio are found instead of silently skipped. Composio's first-time login writes your API key to the Windows Credential Manager via the same FFI-against-native-APIs pattern Qaio uses on the macOS Keychain. No plaintext on disk.
- **Project moved to the getqaio org.** `github.com/ja-818/qaio` is now `github.com/getqaio/qaio`. The old URL still redirects, so existing clones, forks, and updater endpoints keep working.

### Reliability

- **Sessions no longer die at the first message** when an unrelated copy of Claude Code lives elsewhere on your computer. Qaio now talks directly to the version it manages itself, which is always kept in sync with what Qaio was tested against. Previously, an older `claude` install on the same machine could intercept the call and reject newer flags, surfacing as a vague "app error."
...
- **Updater uses the same minisign signing key for both OSes.** The signing key controls who can ship updates to existing Qaio installs, regardless of platform.

### Before you upgrade

- **Mac:** quit Qaio before installing. macOS does not always replace a running app cleanly. If in doubt, remove `/Applications/Qaio.app` and drag the new copy in.
...
Qaio should detect 0.4.6, download it, install it, and relaunch into the new version on its own.

### If you are on 0.4.2 or earlier (Mac)

Auto-update will not pull you forward from those builds. Quit Qaio, drag a fresh DMG over `/Applications/Qaio.app`, and reopen. After that one manual step you are back on the healthy updater path and future versions apply automatically.
