...
Big release. Sign in with Google, every Mac runs the same installer, the app speaks Spanish and Portuguese, your phone can manage Qaio, and the major CLIs your agents use ship preinstalled. No terminal, no extra setup.

### What's new

- **Sign in with Google or Microsoft.** Qaio now has accounts. One click, native OAuth, your session lives in macOS Keychain (never in plain files). Sign out from the sidebar or Settings.
- **Qaio Store with curated agents.** New Store tab ships with team-built agents you can install in one click, with built-in update checks. The store modal is simpler and the GitHub-import detour is gone, so creating an agent feels like adding an app.
...
- **Codex and Composio come bundled.** Both CLIs ship inside the signed Qaio app, so non-technical users get them preinstalled. Claude Code installs itself on first launch (one tap, no terminal). The download is bigger but the setup is one step.
...
- **Functional bug reporter.** When something goes wrong the toast says "Qaio, we have a problem!" and the Report bug button actually sends the report straight to our team in Slack, with your email if you're signed in.
...
- **Mobile companion (beta).** Tap "Connect phone" in the sidebar, scan the QR code with the Qaio iOS app, and you can manage Qaio from your phone. Your Mac stays the source of truth, the phone is a window into it. Skill cards render on the phone too.
...
- **Better link previews.** Sharing getqaio.ai on WhatsApp, Slack, or Twitter now shows a real social card.

### Before you upgrade

- Quit Qaio before installing. macOS doesn't always replace a running app. If in doubt: `rm -rf /Applications/Qaio.app` and drag the new copy in.
