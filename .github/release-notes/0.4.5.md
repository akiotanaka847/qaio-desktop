...
- **Bug reports go straight to Linear.** Send a bug from inside Qaio and it lands as a ticket the team triages, with the diagnostics already attached.
...
- **Recovery from malformed Claude requests.** A specific class of provider error used to leave the chat stuck. Qaio now surfaces a recovery card so you can retry without restarting the agent.
...
- **Anthropic reconnect only fires when auth is confirmed.** Qaio no longer flashes reconnect prompts during the brief window before sign-in finishes resolving.
...
- Quit Qaio before installing. macOS does not always replace a running app cleanly. If in doubt, remove `/Applications/Qaio.app` and drag the new copy in.

### If you are on 0.4.3 or 0.4.4

Qaio should detect 0.4.5, download it, install it, and relaunch into the new version on its own.
