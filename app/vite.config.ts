...
function resolveAuthStorageMode(
  ...
  const override = env.QAIO_AUTH_STORAGE?.trim().toLowerCase();
  if (override === "keychain" || override === "browser") return override;
  if (override) {
    throw new Error("QAIO_AUTH_STORAGE must be keychain or browser");
  }
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      __QAIO_AUTH_STORAGE_MODE__: JSON.stringify(authStorageMode),
      __QAIO_AUTH_STORAGE_SCOPE__: JSON.stringify(authStorageScope),
    },
    clearScreen: false,
