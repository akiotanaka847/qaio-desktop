...
fn launch_app(path: &Path) -> Result<(), String> {
    std::process::Command::new("open")
        ...
        .arg(path)
        .spawn()
        .map_err(|e| format!("Failed to relaunch Qaio: {e}"))?;
    Ok(())
}
...
mod tests {
    ...
    fn resolves_macos_bundle_from_executable_path() {
        let exe = PathBuf::from("/Applications/Qaio.app/Contents/MacOS/Qaio");
        assert_eq!(
            app_path_from_exe(&exe),
            PathBuf::from("/Applications/Qaio.app")
        );
    }
    ...
    fn uses_executable_path_off_macos() {
        let exe = PathBuf::from("/opt/qaio/qaio");
