...
//!
//! Spawns `qaio-engine` as a child process, parses its stdout for the
//! `QAIO_ENGINE_LISTENING port=<p> token=<t>` line, polls `/v1/health`
//! until ready, and hands the `{baseUrl, token}` handshake back to the
//! caller so the Tauri setup can inject `window.__QAIO_ENGINE__` before
//! showing the webview.
//!
//! Lifecycle:
...
//!   backoff and emits a `qaio-event` toast to the webview on each
//!   restart.
//!
...
impl EngineSubprocess {
    /// Spawn `qaio-engine` and wait up to `timeout` for the banner.
    ///
    /// `env` is merged on top of the inherited environment — used by the
    /// Qaio app to pass product-layer prompts (`QAIO_APP_SYSTEM_PROMPT`,
    /// `QAIO_APP_ONBOARDING_PROMPT`) into the engine at boot so the engine
...
/// Resolve the `qaio-engine` binary path.
///
/// Resolution order:
/// 1. `QAIO_ENGINE_BIN` env var (dev override / SSH deploy).
...
///      - macOS: `Qaio.app/Contents/MacOS/qaio-engine`
///      - Windows: next to `qaio-app.exe`
...
/// 4. `<resource_dir>/binaries/qaio-engine` — legacy / belt-and-braces
...
pub fn resolve_engine_binary(resource_dir: Option<&PathBuf>) -> Result<PathBuf, String> {
    ...
    if let Ok(p) = std::env::var("QAIO_ENGINE_BIN") {
    ...
    Err(format!(
        "qaio-engine binary not found. Tried:\n  - {}",
        tried
            .iter()
...
/// Spawn a background thread that keeps `qaio-engine` alive. On crash,
...
fn parse_banner(line: &str) -> Option<EngineHandshake> {
    // Format: QAIO_ENGINE_LISTENING port=<p> token=<t>
    let rest = line.strip_prefix("QAIO_ENGINE_LISTENING ")?;
...
mod tests {
    ...
    #[test]
    fn parses_banner() {
        let h = parse_banner("QAIO_ENGINE_LISTENING port=12345 token=abc").unwrap();
