...
`[Sign in to Composio](https://composio.dev/#qaio_composio_signin=1)`. \
The Qaio chat renders this as a rich sign-in card with a one-click \
...
3. Present the `redirect_url` from that JSON as a markdown link. \
   **IMPORTANT**: append `#qaio_toolkit=<toolkit>` to the URL so \
   the Qaio chat can render it as a rich connect card with live \
   ...
   output exactly: \
   `[Connect Gmail](https://connect.composio.dev/link/lk_abc#qaio_toolkit=gmail)`. \
