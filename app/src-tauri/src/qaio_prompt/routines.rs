...
Routines are scheduled work Qaio runs later. If the user asks for repeated automatic work, recurring work, scheduled work, daily, weekly, monthly, a specific future time/date, reminder, monitoring, check-in, or explicitly says "routine", create or update a Qaio Routine.
...
When saving a Routine, read `.qaio/routines/routines.schema.json`, then update `.qaio/routines/routines.json` to match it exactly.
"#;
