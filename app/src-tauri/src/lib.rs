...
use qaio_tauri::qaio_db::Database;
use qaio_tauri::state::AppState;
...
/// the one-shot `qaio-engine-ready` event fires before the webview's
/// `listen()` registers.
#[derive(Default)]
...
impl SupervisorCallbacks for TauriSupervisorCallbacks {
    fn on_restart(&self, handshake: &EngineHandshake) {
        ...
        let _ = self.handle.emit("qaio-engine-restarted", payload);
        let _ = self.handle.emit(
            "qaio-event",
            QaioEvent::CompletionToast {
...
pub fn run() {
    // Initialize logging before anything else. `qaio_dir()` flips to
    // `~/.dev-qaio/` in debug builds so `pnpm tauri dev` stays isolated
    // from an installed release of Qaio.
    let qaio = qaio_tauri::qaio_db::db::qaio_dir();
    logging::init(&qaio);
    ...
    builder
        ...
        .setup(|app| {
            ...
            // (`qaio://auth-callback?code=...`). Forwards the URL to the
            ...
            qaio_tauri::qaio_terminal_manager::claude_path::init();

            let qaio = qaio_tauri::qaio_db::db::qaio_dir();
            let db_path = qaio.join("db").join("qaio.db");
            ...
            // `~/Documents/Qaio/`. New default is `$QAIO_HOME/workspaces/`
            // so everything Qaio owns is under a single discoverable root.
            // Move the legacy directory if it exists and the new location is
            // empty. Idempotent on subsequent launches.
            migrate_legacy_docs_dir(&qaio);
            ...
            // --- Spawn qaio-engine as a subprocess --------------------
            //
            // All domain calls go through the engine over HTTP/WS. The
            // supervisor thread restarts it with exponential backoff on
            // crash and emits a toast via `qaio-event` on each reconnect.
            let resource_dir = app.path().resource_dir().ok();
            let binary = resolve_engine_binary(resource_dir.as_ref())
                .expect("qaio-engine binary missing — check externalBin bundling");
            tracing::info!("[engine] spawning {}", binary.display());

            let cb: Arc<TauriSupervisorCallbacks> = Arc::new(TauriSupervisorCallbacks {
                handle: app.handle().clone(),
            });
            // Product-layer prompts live in the `qaio_prompt` module and are
            // exported to the engine via env vars. The engine treats these
            // as opaque strings — it has no hardcoded Qaio copy.
            //
            // Also pin QAIO_HOME + QAIO_DOCS so the engine uses the
            // same data roots as the app. Workspaces live under
            // `$QAIO_HOME/workspaces/` in both debug (`~/.dev-qaio/`)
            // and release (`~/.qaio/`) — everything Qaio writes is
            // rooted at a single discoverable location.
            let docs_dir = qaio.join("workspaces");
            let mut engine_env: Vec<(String, String)> = vec![
                (
                    "QAIO_APP_SYSTEM_PROMPT".into(),
                    qaio_prompt::system_prompt(),
                ),
                (
                    "QAIO_APP_ONBOARDING_PROMPT".into(),
                    qaio_prompt::onboarding_prompt(),
                ),
                ("QAIO_HOME".into(), qaio.display().to_string()),
                ("QAIO_DOCS".into(), docs_dir.display().to_string()),
            ];
            ...
            {
                engine_env.push(("QAIO_STORE_DIR".into(), store_dir.display().to_string()));
            }
            ...
            if let Some(user_id) = auth::persisted_user_id() {
                engine_env.push(("QAIO_APP_USER_ID".into(), user_id));
            }
            // Pass through `QAIO_TUNNEL_URL` for local relay dev
            // (`wrangler dev` on localhost:8787). Production uses the
            // engine's baked-in default (`tunnel.getqaio.ai`).
            if let Ok(v) = std::env::var("QAIO_TUNNEL_URL") {
                if !v.is_empty() {
                    engine_env.push(("QAIO_TUNNEL_URL".into(), v));
                }
            }
            ...
            let slot = spawn_supervisor(binary, Duration::from_secs(30), engine_env, cb)
                .expect("failed to spawn qaio-engine");
            let handshake = {
            ...
            // `qaio-engine-ready` event fires before the webview's
            ...
            *handshake_state.0.lock().unwrap() = Some(handshake.clone());
            ...
            // picks it up before any QaioClient call fires.
            //
            ...
            //   2. `qaio-engine-ready` Tauri event — the frontend's
            ...
            let init_script = format!(
                "window.__QAIO_ENGINE__ = {{ baseUrl: \"{}\", token: \"{}\" }};",
                handshake.base_url(),
            ...
            if let Err(e) = app.emit("qaio-engine-ready", ready_payload) {
...
/// `qaio_agent_files::migrate_agent_data` on each. Idempotent — only
/// does real work for agents whose `.qaio/` is still in the legacy flat
...
fn migrate_all_agents(workspaces_root: &std::path::Path) {
    ...
    for ws_entry in entries.flatten() {
        ...
        for agent_entry in agent_entries.flatten() {
            ...
            // Only agents that already have a `.qaio/` dir — otherwise
            // we'd eagerly create one for every random folder in the tree.
            if !agent_path.join(".qaio").is_dir() {
                continue;
            }
            if let Err(e) = qaio_tauri::qaio_agent_files::migrate_agent_data(&agent_path) {
                tracing::warn!(
...
/// Move `~/Documents/Qaio/` to `$qaio/workspaces/` if:
...
fn migrate_legacy_docs_dir(qaio: &std::path::Path) {
    let home = match std::env::var("HOME") {
        Ok(h) => PathBuf::from(h),
        Err(_) => return,
    };
    let legacy = home.join("Documents").join("Qaio");
    let new_root = qaio.join("workspaces");
