...
//!      options: { redirectTo: "qaio://auth-callback" } })`.
//!   2. User completes consent in their system browser.
//!   3. Supabase redirects to `qaio://auth-callback?code=...` (PKCE).
...
const SERVICE: &str = "com.qaio.app.auth";
...
/// stamp `QAIO_APP_USER_ID` on its own operations. Failures (no entry,
...
pub fn persisted_user_id() -> Option<String> {
    if option_env!("QAIO_AUTH_STORAGE_MODE") != Some("keychain") {
        return None;
    }

    // Storage key must match `storageKey` in app/src/lib/supabase.ts.
    let entry = Entry::new(SERVICE, "qaio-auth").ok()?;
...
mod tests {
    ...
    /// with `cargo test -p qaio-app auth:: -- --ignored`.
    #[tokio::test]
    #[ignore]
    async fn keychain_round_trip() {
        let key = "__qaio_test__";
    ...
    fn local_auth_storage_does_not_read_persisted_keychain_user() {
        if option_env!("QAIO_AUTH_STORAGE_MODE") == Some("browser") {
