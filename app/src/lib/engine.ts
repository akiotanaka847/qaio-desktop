/**
 * Engine client bootstrap for the Qaio desktop app.
 *
 * The Tauri supervisor spawns the `qaio-engine` subprocess, parses its
 * stdout for `QAIO_ENGINE_LISTENING port=<p> token=<t>`, and injects
 * `window.__QAIO_ENGINE__ = { baseUrl, token }` via
...
function resolveConfig(): { baseUrl: string; token: string } | null {
  if (typeof window !== "undefined" && window.__QAIO_ENGINE__) {
    return window.__QAIO_ENGINE__;
  }
  // Dev fallback — if QAIO_ENGINE_BASE / TOKEN present on Vite env, use them.
  const baseUrl =
    (import.meta as any).env?.VITE_QAIO_ENGINE_BASE ?? null;
  const token = (import.meta as any).env?.VITE_QAIO_ENGINE_TOKEN ?? null;
  if (baseUrl && token) return { baseUrl, token };
  return null;
}

let _client: QaioClient | null = null;
...
function applyConfig(config: { baseUrl: string; token: string }) {
  window.__QAIO_ENGINE__ = config;
  _client = new QaioClient(config);
  if (_resolveReady) {
    _resolveReady();
    _resolveReady = null;
  }
...
// when the one-shot `qaio-engine-ready` event fires before `listen()`
 ...
 *
 * The Tauri supervisor spawns qaio-engine and emits
 * `qaio-engine-ready` with `{ baseUrl, token }` after /v1/health passes.
...
export function getEngine(): QaioClient {
  if (!_client) {
    throw new Error(
      "[engine] not bootstrapped. window.__QAIO_ENGINE__ missing. " +
...
// --- Tauri event wiring ----------------------------------------------
//
// `qaio-engine-ready` fires ONCE after initial /v1/health passes. This
// is how the frontend learns the port+token when `window.eval` injection
// lost the race against React mount.
//
// `qaio-engine-restarted` fires when the supervisor respawns the
