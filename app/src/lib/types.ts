...
/** Agent category for Qaio Store filtering */
...
export interface AgentMode {
  ...
  promptFile: string;      // Mode name → reads .qaio/prompts/modes/{promptFile}.md
  createLabel: string;     // Button label, e.g. "New Mission"
}

/** The agent config (qaio.json schema) */
export interface AgentConfig {
  ...
  author?: string;         // e.g. "Qaio" for official, user name for community
...
export interface AgentDefinition {
  ...
  path?: string;           // For installed: ~/.qaio/agents/{id}/
...
export interface Agent {
  id: string;
  name: string;
  folderPath: string;      // ~/.qaio/workspaces/{WorkspaceName}/{AgentName}/
...
/** A listing from the Qaio Store registry */
