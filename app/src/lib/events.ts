 ...
 * `engine/qaio-engine-server/src/ws.rs`). Narrower clients (headless
 * agents, mobile) should add targeted subscribe helpers instead of using
 * the firehose so they don't waste bandwidth.
 */

import type { QaioEvent } from "@qaio-ai/core";
import { topics } from "@qaio-ai/engine-client";
...
/**
 * Subscribe to every `QaioEvent` emitted by the backend.
 *
 ...
 */
export function subscribeQaioEvents(handler: (ev: QaioEvent) => void): Unsub {
