...
// at getqaio.ai/auth/callback then forwards the PKCE code into the
// `qaio://auth-callback?code=...` deep link so macOS / Windows hand
// it to the running app. See website/src/auth/callback/index.html.
const REDIRECT_URI = "https://getqaio.ai/auth/callback/";
 ...
 * After consent the browser redirects to `qaio://auth-callback?code=...`,
...
async function signInWithProvider(
  ...
  const { data, error } = await supabase.auth.signInWithOAuth({
    ...
    options: {
      ...
      // since Qaio only needs the email + sub claims for sign-in.
      ...(provider === "azure"
        ? {
