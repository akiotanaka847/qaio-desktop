/**
 * Qaio backend adapter.
 *
 * Every domain call (workspaces, agents, chat, skills, store, sync, …) flows
 * through `@qaio-ai/engine-client` to the `qaio-engine` subprocess the
...
function toAgent(a: import("@qaio-ai/engine-client").Agent): Agent {
  return {
    id: a.id,
    name: a.name,
...
function conversationToRaw(
  c: import("@qaio-ai/engine-client").ConversationEntry,
): RawConversation {
  return {
    id: c.id,
    title: c.title,
