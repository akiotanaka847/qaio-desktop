...
import { SkillDetailPage } from "@qaio-ai/skills";
...
function usePromptFile(agentPath: string, fileName: string) {
  return useQuery({
    ...
    queryFn: () =>
      tauriAgent.readFile(agentPath, `.qaio/prompts/${fileName}`).catch(() => ""),
    enabled: !!agentPath,
  });
}
...
function useSavePromptFile(agentPath: string, fileName: string) {
  return useCallback(
    async (content: string) => {
      await tauriAgent.writeFile(agentPath, `.qaio/prompts/${fileName}`, content);
    },
    [agentPath, fileName],
  );
}
