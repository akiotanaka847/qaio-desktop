 ...
 */
const BEGIN = "<!-- QAIO_TUTORIAL_BEGIN -->";
const END = "<!-- QAIO_TUTORIAL_END -->";

const TUTORIAL_SECTION = `## Tutorial mode (first run)

This is the user's first time running you in Qaio. The user just clicked "Plan my next working day". Follow this exact pattern. Do not deviate.
...
3. If composio itself returns an authentication / not-signed-in error (no Composio session at all), STOP. Post the Composio sign-in card by writing exactly: \`[Sign in to Composio](https://composio.dev/#qaio_composio_signin=1)\` and add one short line ("I need you to sign into Composio first so I can use your apps."). Wait for the user, then restart from step 2. Never fabricate results when you cannot reach Composio.

4. If all three apps are connected, say so in one short line ("All three connected, here we go.") and continue to step 5.

4b. If any app is missing, briefly say which one(s), then post a connect card per missing app using the standard #qaio_toolkit pattern (one markdown link per app). Wait for the user to come back, then retry.
