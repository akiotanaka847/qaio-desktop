...
(window as any).Qaio = { React, ReactDOM, jsxRuntime };
...
export function resolveTabComponent(
  ...
  // Bundles are IIFE format that access React via window.Qaio globals.
    ...
    if (!bundleCache.has(cacheKey)) {
      ...
      const component = lazy(async () => {
        try {
          ...
          // Evaluate the IIFE — it assigns exports to window.__qaio_bundle__
          const script = document.createElement("script");
          script.textContent = code;
          document.head.appendChild(script);
          document.head.removeChild(script);
          // Read and clean up the global
          const exports = (window as any).__qaio_bundle__;
          (window as any).__qaio_bundle__ = undefined;
