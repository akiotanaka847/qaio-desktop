...
import { LEGAL_ACCEPTANCE_KEY, type LegalAcceptance } from "@qaio-ai/engine-client";
...
export interface LegalAcceptanceState {
  ...
  accept: () => Promise<void>;
  /** Close the Qaio window. Called when the user clicks Decline. */
 ...
 * across any surface that shares the same `~/.qaio` DB. We treat the
