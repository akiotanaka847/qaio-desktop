/**
 * Shared helpers for reading/writing typed JSON files under `.qaio/<type>/<type>.json`.
 *
 * Read/write go through `tauriAgent` (in `lib/tauri.ts`) so they respect the
 * `VITE_QAIO_USE_ENGINE_SERVER` flag — same code runs over Tauri IPC today
...
/** Relative path convention: `.qaio/<name>/<name>.json`. */
export function relPath(name: string): string {
  return `.qaio/${name}/${name}.json`;
}

/**
 * Read + parse `.qaio/<name>/<name>.json`. Returns `fallback` when the file
...
/** Serialize + atomically write `.qaio/<name>/<name>.json`. */
