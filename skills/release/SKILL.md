...
7. **Wait CI:** ~10-15 min. Check `github.com/getqaio/qaio/actions`
...
9. **STOP. Hand off draft to user.** The CI-created GH Release is a draft. Tell the user it's ready for review and link it. Do NOT publish it yourself — `gh release edit --draft=false` is the user's call. Publishing flips on auto-update for every installed Qaio, so it's never auto-pilot.
