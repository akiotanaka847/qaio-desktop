...
//
// On failure we show a clear recovery path: open Qaio on the Mac and scan
...
import { QaioHelmet } from "@qaio-ai/core";
...
export function PairScreen() {
  ...
  if (state.kind === "connecting") {
    return (
      <div className="flex min-h-full flex-col items-center justify-center bg-background safe-top px-6 text-center">
        <QaioHelmet color="#737373" size={48} className="mb-4 opacity-70" />
  ...
  if (state.kind === "error") {
    return (
      <div className="flex min-h-full flex-col items-center justify-center bg-background safe-top px-6 text-center">
        <QaioHelmet color="#ef4444" size={48} className="mb-4" />
  ...
  return (
    <div className="flex min-h-full flex-col items-center justify-center bg-background safe-top px-6 text-center">
      <QaioHelmet color="#737373" size={56} className="mb-5" />
      <h1 className="text-2xl font-semibold">Qaio</h1>
      <p className="mt-3 max-w-xs text-sm text-muted-foreground">
        Open <span className="font-medium text-foreground">Qaio on your Mac</span>,
...
function friendlyError(e: unknown): string {
  ...
  switch (e.code) {
    case "code_unknown":
      return "That QR was reset. Open Qaio on your Mac and scan the current QR.";
    case "code_malformed":
      return "The pairing link looks wrong. Open Qaio on your Mac and scan the QR again.";
    case "desktop_offline":
      return "Your Mac is currently offline. Make sure Qaio is open, then tap Try again.";
    case "pair_timeout":
    ...
    case "internal":
    default:
      return "Something went wrong on Qaio\u2019s end. Tap Try again in a moment.";
  }
}
