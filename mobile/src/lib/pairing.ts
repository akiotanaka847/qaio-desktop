// Redeem a phone-access code with the Qaio relay. The code is of the form
...
export interface RelayConfig {
  /** e.g. "https://tunnel.getqaio.ai" or "https://tunnel-staging.getqaio.ai" */
  baseUrl: string;
}

// Mirror of `engine/qaio-engine-server/src/config.rs::DEFAULT_RELAY_URL`.
...
const DEFAULT_RELAY = "https://tunnel.getqaio.ai";
...
/** Stable wire codes mirrored from `qaio-relay/src/types.ts::PairErrorCode`.
...
async function attemptRedeem(
  ...
  try {
    ...
    throw new PairError(
      `Could not reach the Qaio relay. ${e instanceof Error ? e.message : ""}`,
      0,
      "network",
    );
  }
