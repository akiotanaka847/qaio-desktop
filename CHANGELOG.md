...
All notable changes to Qaio will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

Qaio ships as a bundle: the **app** follows the top-level semver
...
- **pptx workspace glob** — `pnpm-workspace.yaml` now includes
  ...
  `@qaio-ai/engine-client` as a workspace dependency
...
- **System-prompt user-voice rules** — every agent now follows explicit
  ...
  `app/src-tauri/src/qaio_prompt.rs::QAIO_SYSTEM_PROMPT`.
...
### Added
- **Standalone `qaio-engine` binary** (engine 0.4.0, protocol v1) —
...
- **`qaio-engine-server` crate** with ~80 REST endpoints across 17
  ...
  module under `engine/qaio-engine-server/tests/`.
- **`qaio-engine-core` crate** — transport-neutral domain logic
...
- **`qaio-engine-protocol` crate** — wire types (REST DTOs, WS
  envelope, error codes, `PROTOCOL_VERSION`). Source of truth for
  `ui/engine-client/src/types.ts`.
- **`@qaio-ai/engine-client`** TypeScript SDK — single npm package
  with `QaioClient` + `EngineWebSocket`. ~1200 lines. The one
  dependency third-party devs install to build a custom frontend.
- **Qaio Always On** — VPS self-host shipping today. Multi-stage
...
- **Engine supervisor** (`app/src-tauri/src/engine_supervisor.rs`) —
  spawns the binary, parses the `QAIO_ENGINE_LISTENING` banner
  for `{port, token}`, injects `window.__QAIO_ENGINE__` into the
...
- **Qaio Mobile** — Capacitor + React PWA companion, deploys to
...
- `.qaio/<type>/<type>.json` typed layout (was flat
  `.qaio/<type>.md` in 0.1.0) with embedded JSON Schemas
...
### Changed
- Moved Rust domain logic from `app/qaio-tauri/src/commands/` into
  standalone crates (`qaio-agent-files`,
  `qaio-agents-conversations`, `qaio-terminal-manager`, …) —
  groundwork for the engine split in 0.3.2
- Made all `.qaio/` writes atomic (temp + rename) with
...
- **ChatSidebar** — progress tracking sidebar showing step checklist + channels list, moved to `@qaio-ai/chat`
- **ProgressPanel** — step-by-step agent progress checklist alongside chat
- **`.qaio/prompts/` convention** — editable system prompt components (`system.md`, `self-improvement.md`)
...
### Added
- Qaio unified app with experience system
...
- 11 React packages (@qaio-ai/core, chat, board, layout, workspace, skills, connections, events, routines, review, memory)
- 8 Rust crates (qaio-sessions, qaio-db, qaio-tauri, qaio-channels, qaio-events, qaio-scheduler, qaio-memory, qaio-skills)
- create-qaio-experience CLI
- Component showcase app

### Changed
- Rebranded from Keel & Deck to Qaio
- Workspace convention: .keel/ → .qaio/
- Events: KeelEvent → QaioEvent

[Unreleased]: https://github.com/getqaio/qaio/compare/v0.3.2...HEAD
[0.3.2]: https://github.com/getqaio/qaio/releases/tag/v0.3.2
[0.3.1]: https://github.com/getqaio/qaio/releases/tag/v0.3.1
[0.3.0]: https://github.com/getqaio/qaio/releases/tag/v0.3.0
[0.2.0]: https://github.com/getqaio/qaio/releases/tag/v0.2.0
[0.1.0]: https://github.com/getqaio/qaio/releases/tag/v0.1.0
