...
Release-bundled registry of Qaio-built agents. No start-from-zero.
...
Installing a listing copies the package into `~/.qaio/agents/<agent-id>/`;
...
Store manifests must not seed `.qaio/activity.json` or
`.qaio/activity/activity.json`. A fresh Store agent should have an
...
Qaio-owned agents update with app releases. Installed definitions
record `.source.json` with `source: "qaio-store"`, `version`, and
...
When a bundled package updates, Qaio also syncs newly-added packaged
...
new Qaio-built workflows appear in existing agents, Skill cards can
...
1. Reads the workspace's last-synced bundled version from
   `.qaio/bundled-package.json` (or treats the install as
...
4. Writes `.qaio/bundled-package.json` with the new version, so
...
- **Qaio App** consumes Store in the New Agent dialog.
