...
|--------|--------|--------------|
| `aarch64-apple-darwin` | ✅ native | `cargo test --workspace --exclude qaio-app --exclude qaio-tauri` |
| `x86_64-apple-darwin` | ✅ (inherits Darwin code paths) | — |
| `x86_64-unknown-linux-gnu` | ✅ (inherits Unix code paths) | — |
| `x86_64-pc-windows-gnu` | ✅ `cargo check` clean | `cargo check --target x86_64-pc-windows-gnu -p qaio-engine-server` with `mingw-w64` toolchain |
...
only in CRT/linker — every `cfg(windows)` branch in Qaio applies to
both.
...
- **Symlinks**: `std::os::unix::fs::symlink` on Unix,
  ...
  `skills.rs`.
- **PTY**: `portable-pty` (used in `qaio-terminal-manager::manager`)
...
| Composio installer | `qaio-composio/src/install.rs::install` | `bash -c "curl \| bash"` | returns a clear error — auto-install not wired (see **gaps** below) |
| Composio executable check | `qaio-composio/src/install.rs::is_installed` | file + `+x` bit | file existence only (NTFS has no POSIX +x; Composio installer drops `composio.exe`) |
| Composio CLI path | same | `~/.composio/composio` | `~/.composio/composio.exe` |
| Login-shell PATH probe | `qaio-terminal-manager/src/claude_path.rs` | `/bin/zsh -l -c 'echo $PATH'`, fallback `/bin/bash -l`, `-i` | skipped — inherited process PATH is already the user PATH |
...
- `qaio-composio::cli`, `::auth`, `::mcp` — these call `whoami`,
...
Wave 2 landed alongside the getqaio/composio fork and the new
...
3. **nvm-windows** — STILL UNVALIDATED. Qaio's bundled CLIs win
...
5. **MSVC target build** — VERIFIED via the new `build-windows` CI
   ...
   qaio-engine-server`). Mac-host cross-compile remains
