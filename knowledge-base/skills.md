...
A Skill is a reusable procedure stored as a markdown file with YAML frontmatter. Qaio shows them in the picker, the chat empty state, and the per-agent Skills tab.
...
Qaio Store agent packages may also include `.agents/skills/*`.
Install copies the package to `~/.qaio/agents/<id>/`; creating a
...
Source of truth: `engine/qaio-skills/src/lib.rs` (`SkillSummary`). Parsed by `serde_yml`, so anything valid YAML works.
...
1. **Engine** parses SKILL.md frontmatter via `serde_yml` (`engine/qaio-skills/src/format.rs`). Unknown fields are silently ignored — old skills with `icon:` / `starter_prompt:` still parse.
...
inline in the Add Skills UI; they should not show global "Qaio problem" bug
...
```
<!--qaio:skill {"skill":"research-company","displayName":"Research a company","image":"...","description":"...","integrations":["tavily"],"fields":[],"message":"Focus on pricing.","attachments":[]}-->
...
- Decoder lives in `@qaio-ai/chat`'s `skill-message.ts` so desktop AND mobile render the same card from the same payload. The decoder also accepts a legacy `<!--qaio:action ...-->` prefix so chat history persisted before the rename keeps rendering as a card.
...
```
<!--qaio:attachments {"message":"Summarize this","files":[{"name":"brief.pdf","path":"/Users/.../brief.pdf"}]}-->
...
- Decoder + shared badge renderer live in `@qaio-ai/chat` (`attachment-message.ts`, `user-attachment-message.tsx`). Desktop encoder lives in `app/src/lib/attachment-message.ts`.
...
2. Write `~/.qaio/workspaces/<Workspace>/<Agent>/.agents/skills/<slug>/SKILL.md` with the full frontmatter schema above.
...
The user never sees the `name` slug — they see `humanize(name)` (e.g. `"Research company"` from `"research-company"`). Qaio's audience is non-technical founders who have never opened a terminal. Pick slugs that **humanize cleanly into a phrase a founder would say in chat**.
...
| What | Where |
|------|-------|
| Schema (Rust) | [`engine/qaio-skills/src/lib.rs`](../engine/qaio-skills/src/lib.rs) |
| Parser / serializer | [`engine/qaio-skills/src/format.rs`](../engine/qaio-skills/src/format.rs) |
| Engine DTO | [`engine/qaio-engine-core/src/skills.rs`](../engine/qaio-engine-core/src/skills.rs) |
...
| System prompt template | [`app/src-tauri/src/qaio_prompt/skills_memory.rs`](../app/src-tauri/src/qaio_prompt/skills_memory.rs) (`SELF_IMPROVEMENT_GUIDANCE`) |
