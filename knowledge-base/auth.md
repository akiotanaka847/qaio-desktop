...
2. Frontend calls `supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: "qaio://auth-callback", skipBrowserRedirect: true } })`.
...
6. Google redirects to Supabase → Supabase redirects to `qaio://auth-callback?code=<code>`.
7. macOS delivers the URL to the running Qaio via `tauri-plugin-deep-link`.
...
| Piece | Where |
|---|---|
| Session JSON (access_token, refresh_token, user) | CI releases: Keychain entry `com.qaio.app.auth` / `qaio-auth` |
| PKCE code verifier | CI releases: Keychain entry `com.qaio.app.auth` / `sb-…-auth-token-code-verifier` (Supabase-managed key) |
...
| Local storage | Browser storage with worktree-scoped key `qaio-auth-local-<hash>` |
...
Local builds are different on purpose: Supabase uses browser storage with a worktree-scoped key (`qaio-auth-local-<hash>`) and the Rust startup path does not read the persisted Keychain user id. macOS can treat every rebuild or worktree as a different app for Keychain access and show repeated password prompts. CI builds keep Keychain storage. Override with `QAIO_AUTH_STORAGE=keychain` or `QAIO_AUTH_STORAGE=browser`.
...
- At engine spawn (`app/src-tauri/src/lib.rs`), CI-release builds read the persisted Supabase session from Keychain and pass `QAIO_APP_USER_ID` as an env var to the subprocess. Local builds skip this Keychain read. Engine treats the value as an opaque string.
...
- `QaioEvent` envelope does NOT carry `user_id` today — deferred until there's a server-side consumer that needs it. When that lands, wrap `QaioEvent` in an envelope struct in `engine/qaio-ui-events` rather than adding `user_id` to each variant.
...
- Server-side Rust emitting PostHog events directly — frontend covers Qaio's event surface.
- Qaio Cloud API endpoints — this is the identity foundation, not the product surface.
...
Separate from Qaio account auth. Claude Code and Codex keep their own CLI
sessions. When those sessions expire mid-chat, `qaio-terminal-manager`
classifies auth-shaped stderr/stdout (`401`, `unauthorized`, `not authenticated`,
expired OAuth/API-key messages) and `qaio-agents-conversations` emits
`QaioEvent::AuthRequired`. Desktop listens in `use-session-events.ts`, sets
...
Never mutate `~/.codex/config.toml` to make Codex read Qaio agent
