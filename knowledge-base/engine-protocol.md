# Qaio Engine — Wire Protocol

Source of truth for the HTTP + WebSocket contract spoken by `qaio-engine`
and every client (desktop, mobile, CLI, third-party). Rust types live in
`engine/qaio-engine-protocol`; TS types live in
...
| Engine version | crate `qaio-engine-server` version |
| Version header | `X-Qaio-Engine-Version: <semver>` on every response |
...
Safari/WKWebView). See `engine/qaio-engine-server/src/lib.rs`.
...
- `Sec-WebSocket-Protocol: qaio-bearer.<token>` — fallback for browser WS.

Token generation: the binary auto-generates a 48-char alphanumeric token on
first run unless `QAIO_ENGINE_TOKEN` is set. It is written (mode 0600) to
`~/.qaio/engine.json`. The desktop supervisor reads that file before
injecting `window.__QAIO_ENGINE__`.
...
Full surface live. Every mutating route emits matching `QaioEvent` on
broadcast bus. 16 route modules wired in
[`qaio-engine-server/src/lib.rs`](../engine/qaio-engine-server/src/lib.rs).
Integration tests in `engine/qaio-engine-server/tests/` — one file per
module.
...
**Agent files** (typed `.qaio/` + project file browser)
| Method | Path | Description |
|---|---|---|
...
| POST | `/v1/agents/files/seed-schemas` | Seed `.qaio/<type>/<type>.schema.json` |
...
| POST | `/v1/store/installs` | Install by `{repo, agentId}`. `repo: "qaio-store/<id>"` installs bundled package incl. skills. GitHub repo form remains supported. |
...
See [`docs/mobile-architecture.md`](../docs/mobile-architecture.md) for the full flow — desktop engine opens an outbound WS to the Qaio relay, which proxies mobile HTTP+WS AND serves the PWA bundle from the same origin. Phone pairing is durable: laptop sleep/shutdown keeps the same tunnel identity and phone tokens; only Settings → Disconnect all phones rotates the QR secret.
...
- `kind: "event"` → `payload` is a `QaioEvent` (same enum the frontend already consumes) or a `LagMarker` (`{type:"Lag", dropped: N}`).
...
- `engine/qaio-engine-server/tests/` — in-process HTTP + WS assertions.
...
app passed in via `QAIO_APP_SYSTEM_PROMPT` at subprocess spawn. The
...
`QAIO_APP_ONBOARDING_PROMPT` as an additional suffix.
...
frontend consumer of the engine, ~400 lines of TSX, zero `@qaio-ai/*`
