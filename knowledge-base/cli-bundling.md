...
Qaio ships two upstream CLIs inside the signed/notarized desktop
...
they install Qaio, click in, and the chat agent works without ever
opening a shell.
...
|-------------|---------------|-------------------|------------------------------------------------------------------------|
| codex       | Apache-2.0    | Bundled (universal) | `Qaio.app/Contents/Resources/bin/codex` — single Mach-O fat binary |
...
2. **composio** is BUILT FROM SOURCE on the Windows runner. Upstream
   ...
   #3057, closed: "use WSL for now"). Qaio builds composio.exe on
   every release using a pinned commit on a forked repo
   (`getqaio/composio`, branch `qaio-windows-support`) that adds:
...
3. **claude-code** ships native Windows binaries (`win32-x64`,
   ...
   runtime installer (`qaio-claude-installer`) detects platform via
...
2. Clones the getqaio/composio fork at the pinned commit SHA
...
`engine/qaio-cli-bundle/` is the resolver crate. Public functions:
...
checking for `Qaio.app/Contents/MacOS/<exe>` (macOS) or a sibling
`resources/bin/` (Windows). No env vars; works even when launched from
Spotlight, Dock, or Finder.

`engine/qaio-terminal-manager/src/claude_path.rs` prepends the
...
`engine/qaio-engine-server/src/main.rs` spawns two background tasks at
boot:

1. `qaio_composio::lifecycle::ensure_and_upgrade` — emits
   ...
   into `~/.composio` and runs `composio upgrade` on Qaio version
   bumps.

2. `qaio_claude_installer::ensure_and_upgrade` — reads
...
- `installSource: "bundled" | "managed" | "path" | "missing"` —
  Qaio's view of where the binary came from. Renders as a chip in
  the provider settings UI.
- `cliPath: string | null` — absolute path Qaio will spawn.
...
4. Add a resolver in `qaio-cli-bundle::lib.rs`
   (`bundled_<name>_path()`).
...
`https://github.com/getqaio/composio.git` on branch
`qaio-windows-support`. Qaio pins the HEAD SHA in
...
1. On the getqaio/composio fork, rebase `qaio-windows-support`
...
3. In Qaio, update `cli-deps.json`:
   ```bash
...
The fork is the deliberate single-point-of-truth for Qaio-side
...
into Qaio). Upstream taking the patches deletes this section.
...
1. SignPath Foundation (free OSS code signing) is approved for Qaio
...
flow. So Windows users on older Qaio versions get the same
...
- `engine/qaio-cli-bundle/` — resolver crate (Windows-aware).
- `engine/qaio-claude-installer/` — runtime download crate
  (Windows install dir at `%LOCALAPPDATA%\Programs\claude\`).
- `engine/qaio-composio/src/install.rs` — bundle-aware composio
  resolver (Windows surfaces a clear "bundled-only" error in dev mode).
- `engine/qaio-engine-core/src/provider.rs` — `InstallSource` enum + status.
- `engine/qaio-terminal-manager/src/claude_path.rs` — PATH
  augmentation, Windows-specific install dirs +
  `.exe`/`.cmd`/`.bat` extension probing.
- `engine/qaio-engine-server/src/routes/claude.rs` — `/v1/claude/*`.
