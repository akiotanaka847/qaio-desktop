# Qaio Engine Server — Operator Guide

`qaio-engine` is the binary that speaks `knowledge-base/engine-protocol.md`.
Everything Qaio can do on a laptop it can do on a VPS — the desktop app
spawns this binary as a subprocess and talks to it the same way a remote
client would.

## Binary

- Crate: `engine/qaio-engine-server`
- Bin target: `qaio-engine`
- Build: `cargo build --release -p qaio-engine-server --bin qaio-engine`
...
| Var | Default | Purpose |
|---|---|---|
| `QAIO_BIND` | `127.0.0.1:0` | `ip:port`. Random local port by default. |
| `QAIO_BIND_ALL` | unset | Must be `1` to bind `0.0.0.0`. Safety net against accidental public exposure. |
| `QAIO_ENGINE_TOKEN` | auto | Bearer token clients must send. 48-char alphanumeric if unset. |
| `QAIO_HOME` | `~/.qaio` | DB, logs, `engine.json`, workspaces. |
| `QAIO_DOCS` | `$QAIO_HOME/workspaces` | Workspaces filesystem root. |
| `QAIO_APP_SYSTEM_PROMPT` | `""` | Product-layer system prompt. Prepended to every session when the caller doesn't supply its own. Set by the embedding app (e.g. Qaio desktop) at subprocess spawn. Engine treats it as an opaque string — no hardcoded product copy in engine source. |
| `QAIO_APP_ONBOARDING_PROMPT` | `""` | Product-layer onboarding suffix. Appended after the system prompt on first-run sessions (`POST /v1/agents/:path/sessions/onboarding`). |
| `QAIO_NO_PARENT_WATCHDOG` | unset | Set to `1` to disable the stdin-EOF → exit watchdog (see "Parent watchdog" below). Required when running under systemd/docker where no supervisor holds the stdin pipe. |
| `RUST_LOG` | `info,qaio=debug` | `tracing` filter. |

## Startup handshake

On bind the binary:

1. Writes `$QAIO_HOME/engine.json` (chmod 0600) with:
   ```json
   {
     "version": "0.4.0",
     "protocol": 1,
...
2. Emits one line to **stdout**:
   ```
   QAIO_ENGINE_LISTENING port=53871 token=<full-token>
   ```
...
`engine/qaio-engine-server/src/main.rs::spawn_parent_watchdog` runs a
...
- Disabled when `QAIO_NO_PARENT_WATCHDOG=1` is set. Use this under
...
1. Pull new binary.
2. `systemctl restart qaio-engine` (or `docker compose up -d`).
...
- **Engine exits immediately on manual run / under non-Tauri supervisor** →
  parent watchdog sees no writer on stdin. Set `QAIO_NO_PARENT_WATCHDOG=1`
  or redirect stdin from a pipe that stays open (`cat | qaio-engine`).
