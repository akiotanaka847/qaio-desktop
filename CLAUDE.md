...
Qaio = desktop app + standalone engine + open library of agents.
...
- **`engine/`** — Rust crates. `qaio-engine-core` = runtime/domain. `qaio-engine-protocol` = wire types. `qaio-engine-server` = axum HTTP+WS binary (`qaio-engine`). `qaio-agent-files`, `qaio-skills`, `qaio-sessions`, `qaio-file-watcher`, etc. are leaf crates. Frontend-agnostic: no Tauri, no React.
- **`ui/`** — `@qaio-ai/*` React packages (chat, board, layout, engine-client, …). Props-only, no store imports. `@qaio-ai/engine-client` is the TS front door to the engine.
- **User data** — `~/.qaio/`: DB, logs, `engine.json`, and `workspaces/<Workspace>/<Agent>/`. Each agent has `.qaio/` data files + `CLAUDE.md` + `.agents/skills/`.
- **Wire contract** — every domain call is a `fetch` or WS frame in `@qaio-ai/engine-client`. There are NO `invoke("list_workspaces", …)` style Tauri commands for domain; those were all deleted.
- **Reactivity** — engine emits `QaioEvent`s; desktop subscribes to the WS `*` firehose; TanStack Query invalidation in `app/src/hooks/use-agent-invalidation.ts` maps events → query keys. File watcher catches direct agent writes.
- **Voice** — agents' target user is NON-technical. The product system prompt forbids mentioning files/JSON/configs/CLIs when talking to the user. Lives in `app/src-tauri/src/qaio_prompt.rs` (Qaio app), NOT in the engine. Engine is prompt-agnostic; app hands it over at spawn via `QAIO_APP_SYSTEM_PROMPT`.
...
- `.qaio/` layout, schemas, reactivity → `knowledge-base/files-first.md`
...
- `qaio-engine` binary ops → `knowledge-base/engine-server.md`
- Bundled CLIs (codex universal, composio per-arch) + runtime claude-code installer → `knowledge-base/cli-bundling.md`
- Custom frontend on `qaio-engine` (integration reference) → `examples/smartbooks/README.md`
...
| engine/ Win check | — | `cargo check --target x86_64-pc-windows-gnu -p qaio-engine-server` (needs mingw-w64) | — |
...
`pnpm tauri dev` spawns the engine as a subprocess from `app/src-tauri/binaries/qaio-engine-<triple>`, which `build.rs` stages from `target/{debug,release}/qaio-engine`. Tauri does NOT rebuild the engine on its own — frontend HMR works fine but the sidecar is whatever binary was last compiled.

**Rule**: any time a PR touches `engine/**` (including merges that bring engine changes from `main`), run `cargo build -p qaio-engine-server` BEFORE the next `pnpm tauri dev` and restart it. Symptoms of a stale sidecar: 404s on routes that exist in the current source, missing event types, schema mismatches. Production users never hit this — release CI builds the engine from scratch on every tag.
...
- Tauri-specific code → `app/qaio-tauri/` (the adapter).

### AI-native reactivity
- Every `.qaio/` data surface must react to file changes regardless of who wrote (user via UI, agent via file write, external edit).
- All `.qaio/` fetching → TanStack Query + event invalidation. No load-on-mount-only.
...
- Qaio ships **en / es / pt**. Every user-facing string flows through `t()` from `react-i18next`. No literal English in JSX text, props, placeholders, aria-labels, toast titles, error messages, or `<Empty>` defaults.
...
- **`ui/@qaio-ai/*` stays i18n-agnostic** per the library boundary. Components take optional `labels?` props with English defaults; the consumer in `app/` passes `t()` results in. Don't import `react-i18next` in `ui/`.
...
- **User data = different.** Canonical location is `~/.qaio/**` (workspaces live at `~/.qaio/workspaces/`). Shape/layout changes inside `~/.qaio/<agent>/.qaio/**` need an **idempotent migration** in `qaio_agent_files::migrate_agent_data`. Never break existing users.
- **Legacy `~/Documents/Qaio/**`** — earlier versions used this path. We do NOT auto-migrate from there; if a user upgrades they may need to copy their workspaces manually. When introducing further root moves, propose a migration story before executing.
...
shadcn/ui registry, @qaio-ai showcase, existing components, npm — before writing from scratch.
