...
Self-host the Qaio Engine on your own server. Your agents keep running
...
cd always-on/
cp .env.example .env
$EDITOR .env   # fill in QAIO_ENGINE_TOKEN
docker compose up -d
...
In Qaio → Settings → Connect to remote engine, paste:

- URL: `https://qaio.example.com`
- Token: `$QAIO_ENGINE_TOKEN`
...
| Var | Default | Purpose |
|---|---|---|
| `QAIO_BIND` | `127.0.0.1:0` | `ip:port` to bind. Set to `0.0.0.0:7777` for remote. |
| `QAIO_BIND_ALL` | unset | Must be `1` to allow binding `0.0.0.0`. |
| `QAIO_ENGINE_TOKEN` | auto-generated | Bearer token clients must send. |
| `QAIO_HOME` | `~/.qaio` | Data dir (DB, `engine.json`, workspaces). |
| `QAIO_DOCS` | `$QAIO_HOME/workspaces` | Workspaces root. |
...
QAIO_BIND=0.0.0.0:7777 QAIO_BIND_ALL=1 \
  QAIO_ENGINE_TOKEN=$TOKEN \
  ./target/release/qaio-engine
```

A systemd unit template lives at `always-on/qaio-engine.service`.

## Updating

The engine exposes its version at `GET /v1/version`. When we release a new
major-minor, the `X-Qaio-Engine-Version` header bumps; desktop clients
